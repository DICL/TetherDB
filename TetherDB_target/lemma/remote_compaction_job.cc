...
int RemoteFindFileInRange(const InternalKeyComparator& icmp,
...
const char* RemoteGetCompactionReasonString(CompactionReason compaction_reason) {
...
struct RemoteCompactionJob::SubcompactionState {
  ...
  struct Output {
    Output(FileMetaData&& _meta,
#if ROCKSDB_SPDK
           uint64_t _start_lpn,
#endif
           const InternalKeyComparator& _icmp,
  ...
  std::vector<std::unique_ptr<WritableFileWriter>> outfile_vector;//lemma_async
...
struct RemoteCompactionJob::CompactionState {
  Compaction* const compaction;

  // REQUIRED: subcompaction states are stored in order of increasing
  // key-range
  std::vector<RemoteCompactionJob::SubcompactionState> sub_compact_states;
  Status status;
...
void RemoteCompactionJob::AggregateStatistics() {
  assert(compact_);
...
RemoteCompactionJob::RemoteCompactionJob(
    ...
    size_t result_file_count, uint64_t result_file_number, int base_level,
#if ROCKSDB_SPDK
    uint64_t* start_lpns,
#endif
    ...
    : job_id_(job_id),
      ...
      result_file_count_(result_file_count-1),
      result_file_number_(result_file_number),
      base_level_(base_level),
#if ROCKSDB_SPDK
      start_lpns_(start_lpns),
#endif
      ...
      paranoid_file_checks_(compaction->mutable_cf_options()->paranoid_file_checks),
      measure_io_stats_(compaction->mutable_cf_options()->report_bg_io_stats),
      thread_pri_(thread_pri),
      full_history_ts_low_(std::move(full_history_ts_low)) {
  result_file_count_og_ = result_file_count;
...
void RemoteCompactionJob::ReportStartedCompaction(Compaction* compaction) {
  const auto* cfd = compact_->compaction->column_family_data();
  ThreadStatusUtil::SetColumnFamily(cfd, cfd->ioptions()->env,
                                    db_options_.enable_thread_tracking);
  ...
  /*assert(compaction->IsTrivialMove() == false ||
         compaction->is_manual_compaction() == true);*///lemmalemma
  ThreadStatusUtil::SetThreadOperationProperty(
...
void RemoteCompactionJob::Prepare() {
  ...
  if (c->column_family_data()->initial_cf_options().compaction_style != kCompactionStyleLevel) {
    write_hint_ = Env::WLTH_NOT_SET;
  } else if (output_level == 0) {
    write_hint_ = Env::WLTH_MEDIUM;
  } else {
    if (output_level - base_level_ >= 2) {
      write_hint_ = Env::WLTH_EXTREME;
    } else if (output_level < base_level_) {
      // There is no restriction which prevents level passed in to be smaller
      // than base_level.
      write_hint_ = Env::WLTH_MEDIUM;
    } else {
      write_hint_ = static_cast<Env::WriteLifeTimeHint>(output_level - base_level_ +
  ...
  if (c->max_subcompactions() > 1) {
    ...
    for (size_t i = 0; i <= boundaries_.size(); i++) {
      ...
      compact_->sub_compact_states.emplace_back(c, start, end, 0);//sizes_[i]);
    }
...
void RemoteCompactionJob::GenSubcompactionBoundaries() {
  auto* c = compact_->compaction;
  ...
  for (auto it = bounds.begin();;) {
    ...
    uint64_t size = ApproximateSize(a, b);
    /*db_mutex_->Unlock();
  ...
  uint64_t max_output_files = static_cast<uint64_t>(std::ceil(
      ...
      MaxFileSizeForLevel(*(c->mutable_cf_options()), out_lvl,
          c->immutable_cf_options()->compaction_style, base_level_,
...
uint64_t RemoteCompactionJob::ApproximateSize(const Slice& start, const Slice& end) {
  auto* c = compact_->compaction;
  const auto& icmp = c->column_family_data()->internal_comparator();
  int start_level = c->start_level();
  int end_level = c->output_level()+1;
  ...
  for (int level = start_level; level < end_level; ++level) {
    const LevelFilesBrief& files_brief = *c->input_levels(level);
    ...
    const int idx_start =
        RemoteFindFileInRange(icmp, files_brief, start, 0,
    ...
    if (icmp.Compare(files_brief.files[idx_end].largest_key, end) < 0) {
      idx_end =
          RemoteFindFileInRange(icmp, files_brief, end, idx_start,
                          static_cast<uint32_t>(files_brief.num_files - 1));
    }
  ...
  const double margin = SizeApproximationOptions().files_size_error_margin;
  if (margin > 0 && total_intersecting_size <
    ...
    // level
    TableCache* table_cache = c->column_family_data()->table_cache();
    const SliceTransform* prefix_extractor = c->mutable_cf_options()->prefix_extractor.get();
    for (const auto file_ptr : first_files) {
      total_full_size += ApproximateSize(icmp, *file_ptr, start, end, prefix_extractor, table_cache);
    }
    ...
    for (const auto file_ptr : last_files) {
      ...
      total_full_size += ApproximateOffsetOf(icmp, *file_ptr, end, prefix_extractor, table_cache);
    }
  }
...
uint64_t RemoteCompactionJob::ApproximateOffsetOf(const InternalKeyComparator& icmp, const FdWithKeyRange& f,
  ...
  if (icmp.Compare(f.largest_key, key) <= 0) {
    ...
    if (table_cache != nullptr) {
      result = table_cache->ApproximateOffsetOf(
          key, f.file_metadata->fd, TableReaderCaller::kCompaction, icmp,
          prefix_extractor);
    }
  }
  return result;
...
uint64_t RemoteCompactionJob::ApproximateSize(const InternalKeyComparator& icmp, const FdWithKeyRange& f,
                                     const Slice& start, const Slice& end,
                                     const SliceTransform* prefix_extractor, TableCache* table_cache) {
  ...
  if (icmp.Compare(f.smallest_key, start) >= 0) {
    ...
    return ApproximateOffsetOf(icmp, f, end, prefix_extractor, table_cache);
  }
  ...
  if (icmp.Compare(f.largest_key, end) < 0) {
    ...
    uint64_t start_offset = ApproximateOffsetOf(icmp, f, start, prefix_extractor, table_cache);
  ...
  return table_cache->ApproximateSize(
      start, end, f.file_metadata->fd, TableReaderCaller::kCompaction, icmp,
      prefix_extractor);
}

Status RemoteCompactionJob::Run(compaction_data::CompactionReply* reply) {
  ...
  for (size_t i = 1; i < compact_->sub_compact_states.size(); i++) {
    thread_pool.emplace_back(&RemoteCompactionJob::ProcessKeyValueCompaction, this,
...
/*  if (status.ok()) {
    thread_pool.clear();
    std::vector<const RemoteCompactionJob::SubcompactionState::Output*> files_output;
    for (const auto& state : compact_->sub_compact_states) {
    ...
    auto verify_table = [&](Status& output_status) {
      while (true) {
        ...
        InternalIterator* iter = cfd->table_cache()->NewIterator(
            ...
            files_output[file_idx]->meta, nullptr,
            prefix_extractor,
            nullptr,
            cfd->internal_stats()->GetFileReadHist(
                compact_->compaction->output_level()),
            TableReaderCaller::kCompactionRefill, nullptr,
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  /*//lemma SetOutputTableProperties 
...
#if DEBUG_PRINT
  fprintf(stderr, "job_id_: %d, total_input %lu\n", job_id_, compact_->compaction->CalculateTotalInputSize());
  for (const auto& sub_compact : compact_->sub_compact_states) {
    fprintf(stderr, "job_id_: %d, sub_compact.total_bytes %lu\n", job_id_, sub_compact.total_bytes);
#else
  for (const auto& sub_compact : compact_->sub_compact_states) {
#endif
    int i = 0; //lemma_async
    for (const auto& out : sub_compact.outputs) {
      sub_compact.outfile_vector[i]->Sync(db_options_.use_fsync); //lemma_async
      sub_compact.outfile_vector[i]->Close(); //lemma_async
      i++;//lemma_async
      //FileMetaData& meta = out.meta;
      ParsedInternalKey s_p, l_p;
      ParseInternalKey(out.meta.smallest.Encode(), &s_p, false);
      ParseInternalKey(out.meta.largest.Encode(), &l_p, false);
      /*for(size_t tmpi = 0; tmpi<out.meta.smallest.Encode().size(); tmpi++) {
        char tmp_c = out.meta.smallest.Encode().data()[tmpi];
      ...
      fprintf(stderr, "\n\n\n");
      for(size_t tmpi = 0; tmpi<out.meta.largest.Encode().size(); tmpi++) {
        char tmp_c = out.meta.largest.Encode().data()[tmpi];
        if((tmp_c <= 'z' && tmp_c >= 'a') || (tmp_c <= 'Z' && tmp_c >= 'A') || (tmp_c <= '9' && tmp_c >= '0'))
          fprintf(stderr, " %c", tmp_c);
        else
          fprintf(stderr, " |%d|", tmp_c);
      ...
      compaction_data::CompactionReply::FileMetaData* added_meta = reply->add_meta();
      //added_meta->mutable_smallest()->set_data(meta.smallest.Encode().data());
      //added_meta->mutable_smallest()->set_size(meta.smallest.Encode().size());
      //added_meta->mutable_largest()->set_data(meta.largest.Encode().data());
      //added_meta->mutable_largest()->set_size(meta.largest.Encode().size());
      added_meta->mutable_smallest()->set_data(s_p.user_key.ToString());
      added_meta->mutable_smallest()->set_seq(s_p.sequence);
      added_meta->mutable_smallest()->set_v_type(s_p.type);
      added_meta->mutable_largest()->set_data(l_p.user_key.ToString());
      added_meta->mutable_largest()->set_seq(l_p.sequence);
      added_meta->mutable_largest()->set_v_type(l_p.type);
      added_meta->set_file_size(out.meta.fd.file_size);
      added_meta->set_packed_number_and_path_id(out.meta.fd.packed_number_and_path_id);
      added_meta->set_largest_seqno(out.meta.fd.largest_seqno);
      added_meta->set_smallest_seqno(out.meta.fd.smallest_seqno);
      added_meta->set_oldest_ancester_time(out.meta.oldest_ancester_time);
      added_meta->set_file_creation_time(out.meta.file_creation_time);
      please_count++;

#if DEBUG_PRINT
/*      ParsedInternalKey kk;
      ParseInternalKey(meta.smallest.Encode(), &kk, false);
      InternalKey kkkk(kk.user_key, kk.sequence, kk.type);
      for(size_t tmpi = 0; tmpi<meta.smallest.Encode().size(); tmpi++) {
        char tmp_c = meta.smallest.Encode().data()[tmpi];
        if((tmp_c <= 'z' && tmp_c >= 'a') || (tmp_c <= 'Z' && tmp_c >= 'A') || (tmp_c <= '9' && tmp_c >= '0'))
          fprintf(stderr, " %c %c", tmp_c, kkkk.Encode().data()[tmpi]);
        else
          fprintf(stderr, " |%d %d|", tmp_c, kkkk.Encode().data()[tmpi]);
      }
      fprintf(stderr, "\n");*/
#endif
#if ROCKSDB_SPDK
      added_meta->set_start_lpn(out.start_lpn);
#endif
    }
  }
  if(result_file_count_og_ < please_count)
    fprintf(stderr, "\nFOUND!!!!!!!!!!!!!!!!!! %lu %lu\n\n", result_file_count_og_, please_count);
  if(please_count == 0)
    fprintf(stderr, "\nFOUND!!!!!!!!!!!!!!!!!! 22%lu %lu\n\n", result_file_count_og_, please_count);
  reply->set_result_file_count(result_file_count_+1);
  //CleanupCompaction();
  return status;
}

Status RemoteCompactionJob::Install(const MutableCFOptions& mutable_cf_options) {
  assert(compact_);
...
void RemoteCompactionJob::RemoteInstall(RemoteVersion& version_files) {
  for (const auto& sub_compact : compact_->sub_compact_states) {
    for (const auto& out : sub_compact.outputs) {
      version_files[out.meta.fd.GetNumber()] = std::make_shared<FileMetaData>(out.meta);
    }
  }
  CleanupCompaction();
}

void RemoteCompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {//lemmalemma
  //fprintf(stderr, "\n!!! CompactionJob::ProcessKeyValueCompaction 11\n"); //lemma
  assert(sub_compact);
  ...
  //fprintf(stderr, "\n!!! CompactionJob::ProcessKeyValueCompaction 22\n"); //lemma
  ...
  //fprintf(stderr, "\n!!! CompactionJob::ProcessKeyValueCompaction 33\n"); //lemma
  Status status;
  ...
  //fprintf(stderr, "\n!!! CompactionJob::ProcessKeyValueCompaction 44\n"); //lemma

  while (status.ok() && !cfd->IsDropped() && c_iter->Valid()) { //lemmalemma 
  ...
  //fprintf(stderr, "\n!!! CompactionJob::ProcessKeyValueCompaction 99\n"); //lemma
  ...
  //fprintf(stderr, "\n!!! CompactionJob::ProcessKeyValueCompaction 10\n"); //lemma
  ...
  //fprintf(stderr, "\n!!! CompactionJob::ProcessKeyValueCompaction 11 11\n"); //lemma
  ...
  //fprintf(stderr, "\n!!! CompactionJob::ProcessKeyValueCompaction 12\n"); //lemma
}

void RemoteCompactionJob::RecordDroppedKeys(
...
Status RemoteCompactionJob::FinishCompactionOutputFile(
  ...
  sub_compact->outfile_vector.push_back(std::move(sub_compact->outfile));//lemma_async
  ...
  if (s.ok() && current_entries == 0 && tp.num_range_deletions == 0) {
    ...
    fprintf(stderr, "DeleteFile 22%s\n", fname.c_str());
    if (!ds.ok()) {
      ROCKS_LOG_WARN(
...
Status RemoteCompactionJob::InstallCompactionResults(
    const MutableCFOptions& mutable_cf_options) {
  assert(compact_);
  ...
  for(size_t which = 0; which < compaction->num_input_levels(); which++){
    size_t level_num = compaction->input_levels(which)->num_files;
    input_count += level_num;
    for(size_t i=0; i<level_num; i++){
      input_size += compaction->input(which, i)->fd.GetFileSize();
    }
  }
...
void RemoteCompactionJob::RecordCompactionIOStats() {
...
Status RemoteCompactionJob::OpenCompactionOutputFile(
    SubcompactionState* sub_compact) {
  assert(sub_compact != nullptr);
  assert(sub_compact->builder == nullptr);
  // no need to lock because VersionSet::next_file_number_ is atomic
  uint64_t file_number = result_file_number_.fetch_add(1);////is atomic. 
  std::string fname =
      TableFileName(sub_compact->compaction->immutable_cf_options()->cf_paths,
  ...
  Status s;
#if ROCKSDB_SPDK
  uint64_t start_lpn = start_lpns_[result_file_count_.fetch_sub(1)];
  //if (start_lpn > 214748364)
  //  fprintf(stderr, "compaction !!!!!!!!!!! %lu %lu\n", start_lpn, result_file_count_.load());
#if LEVEL_CACHE
  IOStatus io_s =
      db_options_.spdk_fs->NewWritableFileFromFree(fname, &writable_file, sub_compact->compaction->SpdkOutputFilePreallocationSize(), start_lpn, sub_compact->compaction->output_level());//lemma
#else
  IOStatus io_s =
      db_options_.spdk_fs->NewWritableFileFromFree(fname, &writable_file, sub_compact->compaction->SpdkOutputFilePreallocationSize(), start_lpn);//lemma
#endif
#else
  IOStatus io_s =
...
void RemoteCompactionJob::CleanupCompaction() {
...
void RemoteCompactionJob::UpdateCompactionStats() {
  assert(compact_);
...
void RemoteCompactionJob::UpdateCompactionInputStatsHelper(int* num_files,
                                                     uint64_t* bytes_read,
                                                     int input_level) {
...
void RemoteCompactionJob::UpdateCompactionJobStats(
...
void RemoteCompactionJob::LogCompaction() {
  Compaction* compaction = compact_->compaction;
  ...
  if (db_options_.info_log_level <= InfoLogLevel::INFO_LEVEL) {
    ...
    stream << "job" << job_id_ << "event"
           << "compaction_started"
           << "compaction_reason"
           << RemoteGetCompactionReasonString(compaction->compaction_reason());
