...
struct spdk_env_opts* opts_ = nullptr;
...
static bool probe_cb(void *cb_ctx, const struct spdk_nvme_transport_id *trid,
	...
	printf("Attaching to %s\n", trid->traddr);
        if(cb_ctx && opts) printf("\n");
        fprintf(stderr, "ssdlogging !!!!!!!!!!!!!!!!!!!!!! yes !!!!!!!!!\n");
...
struct SPDKInfo * InitSPDK(std::string device_addr, int logging_queue_num){
	...
	if (opts_ == nullptr) {
	 	opts_ = new struct spdk_env_opts;
		spdk_env_opts_init(opts_);
		fprintf(stderr, "SPDK ENV: %s\n", opts_->name);
		fprintf(stderr, "SPDK ENV: %d %d %d %d\n", opts_->shm_id, opts_->mem_channel, opts_->main_core, opts_->mem_size);
		fprintf(stderr, "SPDK ENV: %lu %s \n", opts_->num_pci_addr, opts_->iova_mode);
		//opts_->shm_id = 0;  //shared memory group ID
		if (spdk_env_init(opts_) < 0) {
		...
		if ( sched_setaffinity(0, sizeof(mask), &mask) ) {
			fprintf(stderr, "fail to set cpu\n");
		}
		else {
			printf("cpu set\n");
		}
