...
//std::atomic<int> job_compaction_only_;
...
class GreeterServiceSingle {
 public:
  ...
  grpc::Status Caching(ServerContext* context, const CacheRequest* request,
    ...
    //thread.detach();
#if ROCKSDB_SPDK
    if(tokens[getIP(context->peer())].load(std::memory_order_relaxed)) {
      cache_mtx_.lock();
  ...
  grpc::Status SayHello(ServerContext* /*context*/, const CompactionRequest* request,
    ...
    //job_compaction_only_ += max_subcompactions;
    //fprintf(stderr, "%d\n", job_compaction_.load(std::memory_order_relaxed));
...
//#endif
  ...
  grpc::Status Preprocess(ServerContext* /*context*/, const PreprocessRequest* request,
    ...
    int PREPROCESS_T_NUM = 4;
    job_compaction_+=4;
    for (int i = 0; i < request->req_size(); i++)
      reply->add_bmps("");
    std::vector<std::thread> ts;
    for (int i = 1; i < PREPROCESS_T_NUM; i++)
      ts.push_back(std::thread(&GreeterServiceSingle::PreprecessJob, this, request, reply, i, PREPROCESS_T_NUM));
    PreprecessJob(request, reply, 0, PREPROCESS_T_NUM);
    for (int i = 0; i < PREPROCESS_T_NUM-1; i++)
      ts[i].join();
    job_compaction_-=4;
    return grpc::Status::OK;
  }

  void PreprecessJob(const PreprocessRequest* request, PreprocessReply* reply, int tid, int tnum) {
    for (int idx = (request->req_size()*tid)/tnum; idx < (request->req_size()*(tid+1))/tnum; idx++) {
      auto req = request->req(idx);
#if ROCKSDB_SPDK
      ...
      spdk_fs_->NewSequentialFile(input_filename, foption, &f, nullptr);
      rocksdb::Status status;
      inputImage.reserve(req.size()); // 
      while(1) {
...
/*      std::vector<uint8_t> inputImage;
      ...
      reply->set_bmps(idx, outputImage.data(), outputImage.size());
    }
  }
...
class GreeterServiceImpl final : public Greeter::Service {
 public:
  ...
  grpc::Status SayHello(ServerContext* context, const CompactionRequest* request,
    ...
    //if(tokens[getIP(context->peer())].load(std::memory_order_relaxed) && job_compaction_.load(std::memory_order_relaxed) < 12) {//pers
    return services[getIP(context->peer())]->SayHello(context, request, reply);
    //} else {
      //return grpc::Status::OK;
    //}
  ...
  //grpc::Status Preprocess(ServerContext* /*context*/, const PreprocessRequest* /*request*/, PreprocessReply* /*reply*/) {
  grpc::Status Preprocess(ServerContext* context, const PreprocessRequest* request, PreprocessReply* reply) {
    /*if(tokens[getIP(context->peer())].load(std::memory_order_relaxed)) {
    //if(tokens[getIP(context->peer())]) {
      return services[getIP(context->peer())]->Preprocess(context, request, reply);
    } else {
      reply->set_sec(last_sleep.tv_sec+TOKEN_TIME);
      reply->set_usec(last_sleep.tv_usec);
      return grpc::Status::OK;
    }*/
    if (job_compaction_.load(std::memory_order_relaxed) < 16) {
      return services[getIP(context->peer())]->Preprocess(context, request, reply);
    } else {
      return grpc::Status::OK;
    }
  ...
  grpc::Status Token(ServerContext* context, const TokenRequest* request,
    ...
    if(request->get()) {
      ...
      tokens[getIP(context->peer())] = true;
 ...
 private:
  ...
  bool ShouldRemote(ServerContext* /*context*/, const void* /*request*/, int /*type*/) {
    ...
    //if (tokens[getIP(context->peer())].load())
      //if ((type == 1 && job_compaction_only_ < 16) || ((const CompactionRequest*)request)->output_level_() < 2)
        //return true;
...
#else
      std::string fname = TableFileName(service->cf_options_.cf_paths, meta->fd.GetNumber(), meta->fd.GetPathId());
#endif
#if DEBUG_PRINT
...
#else
      IOStatus s = service->immutable_db_options_.fs->NewSequentialFile(fname, service->file_options_for_compaction_, &f, nullptr);
#endif
      if (!s.ok())
        continue;
      Slice buffer;
  ...
  void token_handle() {
    ...
    while(1) {
      ...
      gettimeofday(&last_sleep, nullptr);
      ...
      ip_lock.unlock();
      usleep(t);
      /*tokens["10.0.0.4"] = true;
      tokens["10.0.0.5"] = true;
      tokens["10.0.0.6"] = false;
      tokens["10.0.0.10"] = false;
      tokens["11.0.0.2"] = true;
      tokens["11.0.0.7"] = true;
      tokens["11.0.0.8"] = false;
      tokens["11.0.0.9"] = false;
      gettimeofday(&last_sleep, nullptr);
      usleep(t);

      tokens["10.0.0.4"] = false;
      tokens["10.0.0.5"] = false;
      tokens["10.0.0.6"] = true;
      tokens["10.0.0.10"] = true;
      tokens["11.0.0.2"] = false;
      tokens["11.0.0.7"] = false;
      tokens["11.0.0.8"] = true;
      tokens["11.0.0.9"] = true;
      gettimeofday(&last_sleep, nullptr);
      usleep(t);*/
    }
  }
...
void RunServer(int queue_start, int cache_size, int mode = 0) {
  ...
  std::string spdk_conf2 = spdk_conf_d1 + "1";
  std::string spdk_conf4 = spdk_conf_d1 + "2";
  std::string spdk_conf5 = spdk_conf_d1 + "3";
  std::string spdk_conf6 = spdk_conf_d1 + "4";
  std::string spdk_conf7 = spdk_conf_d2 + "5";
  std::string spdk_conf8 = spdk_conf_d2 + "6";
  std::string spdk_conf9 = spdk_conf_d2 + "7";
  std::string spdk_conf10 = spdk_conf_d2 + "8";*/
  /*service.AddService("11.0.0.2", spdk_conf10, queue_start, cache_size, mode);
  ...
  service.AddService("10.0.0.10", spdk_conf2, queue_start, cache_size, mode);*/

  std::string spdk_conf_d2("trtype:RDMA adrfam:IPv4 traddr:11.0.0.91 trsvcid:4421 subnqn:nqn.2019-04.pos:subsystem");
  std::string spdk_conf = spdk_conf_d2 + "1";
  service.AddService("11.0.0.7", spdk_conf, queue_start, cache_size, mode);
