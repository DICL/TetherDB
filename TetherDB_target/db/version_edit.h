...
#if LEMMA_PATH
constexpr uint64_t kFileNumberMask = 0x1FFFFFFFFFFFFFFF;//lemma_path
#else
constexpr uint64_t kFileNumberMask = 0x3FFFFFFFFFFFFFFF;//lemma_path
#endif
...
struct FileMetaData {
  ...
  FileMetaData(const FileMetaData& copy)
    : fd(copy.fd), smallest(copy.smallest), largest(copy.largest),
      table_reader_handle(copy.table_reader_handle), stats(copy.stats),
      compensated_file_size(copy.compensated_file_size),
      num_entries(copy.num_entries), num_deletions(copy.num_deletions),
      raw_key_size(copy.raw_key_size), raw_value_size(copy.raw_value_size),
      refs(copy.refs), being_compacted(copy.being_compacted),
      init_stats_from_file(copy.init_stats_from_file),
      marked_for_compaction(copy.marked_for_compaction),
      oldest_blob_file_number(copy.oldest_blob_file_number),
      oldest_ancester_time(copy.oldest_ancester_time),
      file_creation_time(copy.file_creation_time),
      file_checksum(copy.file_checksum),
      file_checksum_func_name(copy.file_checksum_func_name),
      log_sst(copy.log_sst) {
    if(log_sst != nullptr) {
      log_sst->LogSstRef();
    }
  }
  ...
  FileMetaData& operator=(const FileMetaData& copy) {
    fd = copy.fd;
    smallest = copy.smallest;
    largest = copy.largest;
    table_reader_handle = copy.table_reader_handle;
    stats = copy.stats;
    compensated_file_size = copy.compensated_file_size;
    num_entries = copy.num_entries;
    num_deletions = copy.num_deletions;
    raw_key_size = copy.raw_key_size;
    raw_value_size = copy.raw_value_size;
    refs = copy.refs;
    being_compacted = copy.being_compacted;
    init_stats_from_file = copy.init_stats_from_file;
    marked_for_compaction = copy.marked_for_compaction;
    oldest_blob_file_number = copy.oldest_blob_file_number;
    oldest_ancester_time = copy.oldest_ancester_time;
...
class VersionEdit {
 public:
  ...
  void AddWal(WalNumber number, WalMetadata metadata = WalMetadata()) {
    fprintf(stderr, "(((((((((((((((((((((((((((((((((((((((((((((((((((((( AddWal called %lu\n", number);
