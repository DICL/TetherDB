...
Status DB::Open(const DBOptions& db_options, const std::string& dbname,
  ...
  db_options1.SetSpdkEnv("trtype:PCIe traddr:0000:86:00.0", 0, 20);//lemma
...
Status DBImpl::Open(const DBOptions& db_options, const std::string& dbname,
  ...
  printf("DBImpl::Open !!! 111\n");//lemma
  Status s = ValidateOptionsByTable(db_options, column_families);
  printf("DBImpl::Open !!! 222\n");//lemma
  if (!s.ok()) {
    return s;
  }
  printf("DBImpl::Open !!! 333\n");//lemma

  s = ValidateOptions(db_options, column_families);
  printf("DBImpl::Open !!! 444\n");//lemma
  if (!s.ok()) {
    return s;
  }
  printf("DBImpl::Open !!! 555\n");//lemma

  *dbptr = nullptr;
  handles->clear();
  printf("DBImpl::Open !!! 666\n");//lemma

  size_t max_write_buffer_size = 0;
  printf("DBImpl::Open !!! 777\n");//lemma
  for (auto cf : column_families) {
    max_write_buffer_size =
        std::max(max_write_buffer_size, cf.options.write_buffer_size);
  }

  printf("DBImpl::Open !!! 888\n");//lemma
  DBImpl* impl = new DBImpl(db_options, dbname, seq_per_batch, batch_per_txn);
  printf("DBImpl::Open !!! 999\n");//lemma
  s = impl->env_->CreateDirIfMissing(impl->immutable_db_options_.wal_dir);
  printf("DBImpl::Open !!! 101010\n");//lemma
  if (s.ok()) {
