...
Compaction::Compaction(ColumnFamilyData* _cfd,
                       int _output_level,
                       uint64_t _max_output_file_size,
                       uint64_t _max_compaction_bytes,
                       uint32_t _max_subcompactions,
                       ImmutableCFOptions _immutable_cf_options,
                       MutableCFOptions _mutable_cf_options,
                       int _number_levels, uint32_t _output_path_id,
                       CompressionType _output_compression,
                       CompressionOptions _output_compression_opts,
                       double _score,
                       std::vector<CompactionInputFiles> _inputs,
                       std::vector<FileMetaData*> _grandparents,
                       bool _bottommost_level, bool _is_full_compaction,
                       bool _is_manual_compaction,
                       Slice _smallest_user_key, Slice _largest_user_key,
                       CompactionReason _compaction_reason)
//  : input_vstorage_LevelFiles_(_input_vstorage_LevelFiles),
  : start_level_(_inputs[0].level),
    output_level_(_output_level),
    max_output_file_size_(_max_output_file_size),
    max_compaction_bytes_(_max_compaction_bytes),
    max_subcompactions_(_max_subcompactions),
    immutable_cf_options_(_immutable_cf_options),
    mutable_cf_options_(_mutable_cf_options),
    ...
    //edit_(c->edit()),
    number_levels_(_number_levels),
    cfd_(_cfd),
    output_path_id_(_output_path_id),
    output_compression_(_output_compression),
    output_compression_opts_(_output_compression_opts),
    deletion_compaction_(false),
    inputs_(_inputs),
    grandparents_(_grandparents),
    score_(_score),
    bottommost_level_(_bottommost_level),
    is_full_compaction_(_is_full_compaction),
    is_manual_compaction_(_is_manual_compaction),
    smallest_user_key_(_smallest_user_key),
    largest_user_key_(_largest_user_key),
    compaction_reason_(_compaction_reason) {
  MarkFilesBeingCompacted(true);
  /*for(auto i : _input_vstorage_LevelFiles){
    input_vstorage_LevelFiles_.push_back(i);
  }*/
  for (auto i : inputs_){
    ...
    for(size_t j=0; j<num_files; j++){
      ...
      level_files_brief.files[j].fd = i.files[j]->fd;
      level_files_brief.files[j].file_metadata = i.files[j];
    }
