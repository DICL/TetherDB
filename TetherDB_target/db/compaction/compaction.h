...
struct AtomicCompactionUnitBoundary {
  ...
  AtomicCompactionUnitBoundary(InternalKey* s, InternalKey* l) : smallest(s), largest(l){};
};
...
class Compaction {
 public:
  ...
  Compaction(ColumnFamilyData* _cfd,
             int _output_level,
             ...
             CompactionReason _compaction_reason = CompactionReason::kUnknown);
...
/*  //lemma
 ...
 private:
  ...
  uint64_t min_input_file_oldest_ancester_time_;//lemma
  //std::vector<std::vector<FileMetaData*>> input_vstorage_LevelFiles_;//lemma
