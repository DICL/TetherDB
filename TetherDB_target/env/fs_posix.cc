 ...
 public:
  ...
  IOStatus NewSequentialFile(const std::string& fname,
    ...
    do {
      ...
      //fprintf(stderr, "@@@@@@@@@@@@@@@@@@ open called 111 %s \n", fname.c_str());//lemma
    ...
    if (options.use_direct_reads && !options.use_mmap_reads) {
      ...
      do {
        IOSTATS_TIMER_GUARD(open_nanos);
        //fprintf(stderr, "@@@@@@@@@@@@@@@@@@ fdopen called fdfd %s \n", fname.c_str());//lemma
        file = fdopen(fd, "r");
  ...
  IOStatus NewRandomAccessFile(const std::string& fname,
    ...
    do {
      IOSTATS_TIMER_GUARD(open_nanos);
      //flags |= O_RDREMOTE;
      //fprintf(stderr, "@@@@@@@@@@@@@@@@@@ open called 222 1 %s \n", fname.c_str());//lemma
      fd = open(fname.c_str(), flags, GetDBFileMode(allow_non_owner_access_));
      //fprintf(stderr, "@@@@@@@@@@@@@@@@@@ open called 222 2 %s fd: %d \n", fname.c_str(), fd);//lemma
    } while (fd < 0 && errno == EINTR);
    if (fd < 0) {
      fprintf(stderr, "@@@@@@@@@@@@@@@@@@ open called 222 error %s \n", fname.c_str());//lemma
  ...
  virtual IOStatus OpenWritableFile(const std::string& fname,
    ...
    do {
      IOSTATS_TIMER_GUARD(open_nanos);
      //fprintf(stderr, "@@@@@@@@@@@@@@@@@@ open called 333 %s \n", fname.c_str());//lemma
      fd = open(fname.c_str(), flags, GetDBFileMode(allow_non_owner_access_));
    } while (fd < 0 && errno == EINTR);
  ...
  IOStatus ReuseWritableFile(const std::string& fname,
    ...
    do {
      IOSTATS_TIMER_GUARD(open_nanos);
      //fprintf(stderr, "@@@@@@@@@@@@@@@@@@ open called 444 %s %s \n", old_fname.c_str(), fname.c_str());//lemma
  ...
  IOStatus NewRandomRWFile(const std::string& fname, const FileOptions& options,
    ...
    while (fd < 0) {
      ...
      //fprintf(stderr, "@@@@@@@@@@@@@@@@@@ open called 555 %s \n", fname.c_str());//lemma
      fd = open(fname.c_str(), flags, GetDBFileMode(allow_non_owner_access_));
      if (fd < 0) {
  ...
  IOStatus NewMemoryMappedFileBuffer(
    ...
    while (fd < 0) {
      IOSTATS_TIMER_GUARD(open_nanos);
      //fprintf(stderr, "@@@@@@@@@@@@@@@@@@ open called 666 %s \n", fname.c_str());//lemma
  ...
  IOStatus NewDirectory(const std::string& name, const IOOptions& /*opts*/,
    ...
    {
      IOSTATS_TIMER_GUARD(open_nanos);
      //fprintf(stderr, "@@@@@@@@@@@@@@@@@@ open called 777 %s \n", name.c_str());//lemma
  ...
  IOStatus NewLogger(const std::string& fname, const IOOptions& /*opts*/,
    ...
    {
      IOSTATS_TIMER_GUARD(open_nanos);
      //fprintf(stderr, "@@@@@@@@@@@@@@@@@@ fopen called fff %s \n", fname.c_str());//lemma
      f = fopen(fname.c_str(),
                "w"
    ...
    if (f == nullptr) {
      ...
      int fd = fileno(f);
#ifdef ROCKSDB_FALLOCATE_PRESENTT
  ...
  IOStatus DeleteFile(const std::string& fname, const IOOptions& /*opts*/,
                      IODebugContext* /*dbg*/) override {
    fprintf(stderr, "DeleteFile 11%s\n", fname.c_str());
    IOStatus result;
  ...
  IOStatus LockFile(const std::string& fname, const IOOptions& /*opts*/,
    ...
    *lock = nullptr;
    ...
    {
      IOSTATS_TIMER_GUARD(open_nanos);
      //fprintf(stderr, "@@@@@@@@@@@@@@@@@@ open called 888 %s \n", fname.c_str());//lemma
      fd = open(fname.c_str(), flags, 0644);
    }
    if (fd < 0) {
  ...
  IOStatus IsDirectory(const std::string& path, const IOOptions& /*opts*/,
    ...
    {
      IOSTATS_TIMER_GUARD(open_nanos);
      //fprintf(stderr, "@@@@@@@@@@@@@@@@@@ open called 999 %s \n", path.c_str());//lemma
