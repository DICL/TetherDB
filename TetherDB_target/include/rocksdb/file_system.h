...
class FileSystemWrapper : public FileSystem {
 public:
  ...
  IOStatus AddInMeta(const std::string& fname,
    ...
    return target_->AddInMeta(fname, start_lpn, end_lpn, size, level);
#else
  ...
  IOStatus NewWritableFileFromFree(const std::string& fname,
    ...
    return target_->NewWritableFileFromFree(fname, result, pre_allocate_size, start_lpn, level);
#else
...
FileSystem* NewSpdkEnv(const std::shared_ptr<FileSystem>& base_fs, Env* base_env, std::string pcie_addr, int open_mod, int queue_start, int cache_size, int MULTI_NUM, int MY_ID);
#endif
