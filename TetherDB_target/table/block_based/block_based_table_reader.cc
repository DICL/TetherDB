...
Status BlockBasedTable::Open(
  ...
  if (!ioptions.allow_mmap_reads) {
    ...
    if (!s.ok()) {
      printf("BlockBasedTable::Open status no!!!!!!!!!! 111\n");//lemma_tmp
      return s;
    }
  } else {
  ...
  if (!s.ok())//lemma_tmp
    printf("BlockBasedTable::Open status no!!!!!!!!!! 222\n");//lemma_tmp
  if (!s.ok()) {
    return s;
  }
  if (!BlockBasedTableSupportedVersion(footer.version())) {
    printf("BlockBasedTable::Open status no!!!!!!!!!! 333\n");//lemma_tmp
  ...
  s = new_table->ReadMetaIndexBlock(ro, prefetch_buffer.get(), &metaindex,
                                    &metaindex_iter);
  if (!s.ok())//lemma_tmp
    printf("BlockBasedTable::Open status no!!!!!!!!!! 444\n");//lemma_tmp
  if (!s.ok()) {
    return s;
  }
  ...
  if (!s.ok())//lemma_tmp
    printf("BlockBasedTable::Open status no!!!!!!!!!! 555\n");//lemma_tmp
  if (!s.ok()) {
    return s;
  }
  s = new_table->ReadRangeDelBlock(ro, prefetch_buffer.get(),
                                   ...
                                   &lookup_context);
  if (!s.ok())//lemma_tmp
    printf("BlockBasedTable::Open status no!!!!!!!!!! 666\n");//lemma_tmp
  if (!s.ok()) {
    return s;
  }
  ...
  if (!s.ok())//lemma_tmp
    printf("BlockBasedTable::Open status no!!!!!!!!!! 777\n");//lemma_tmp
...
Status BlockBasedTable::PutDataBlockToCache(
    ...
    GetContext* get_context) const {
  fprintf(stderr, "PutDataBlockToCache\n");//lemma
