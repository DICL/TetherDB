...
Status ReadFooterFromFile(const IOOptions& opts, RandomAccessFileReader* file,
  ...
  if (file_size < Footer::kMinEncodedLength) {
    printf("ReadFooterFromFile status no!!!!!!!!!! 111 %lu\n", enforce_table_magic_number);//lemma_tmp
  ...
  if (prefetch_buffer == nullptr ||
    ...
    if (file->use_direct_io()) {
      ...
      if (!s.ok())//lemma_tmp
        printf("ReadFooterFromFile status no!!!!!!!!!! 222\n");//lemma_tmp
    } else {
      footer_buf.reserve(Footer::kMaxEncodedLength);
      s = file->Read(opts, read_offset, Footer::kMaxEncodedLength,
                     &footer_input, &footer_buf[0], nullptr);
      if (!s.ok())//lemma_tmp
        printf("ReadFooterFromFile status no!!!!!!!!!! 333\n");//lemma_tmp
    }
    if (!s.ok()) return s;
  }
  ...
  if (footer_input.size() < Footer::kMinEncodedLength) {
    printf("ReadFooterFromFile status no!!!!!!!!!! 444\n");//lemma_tmp
    return Status::Corruption("file is too short (" + ToString(file_size) +
                              " bytes) to be an "
                              "sstable" +
                              file->file_name());
  ...
  if (!s.ok()) {
    printf("ReadFooterFromFile status no!!!!!!!!!! 555\n");//lemma_tmp
    return s;
  }
  if (enforce_table_magic_number != 0 &&
      enforce_table_magic_number != footer->table_magic_number()) {
    printf("ReadFooterFromFile status no!!!!!!!!!! 666\n");//lemma_tmp
