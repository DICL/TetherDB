...
#undef DeleteFile
...
#if ROCKSDB_SPDK
  virtual void SpdkMigration(std::string /*dbpath*/, std::string /*data_path*/) {}

  virtual IOStatus GetFileAddr(const std::string& /*fname*/,
                               uint64_t* /*start_lpn*/,
                               uint64_t* /*end_lpn*/) {
    return IOStatus::NotSupported("GetFileAddr is not supported for this FileSystem");
  }
  ...
  virtual IOStatus AddInMeta(const std::string& /*fname*/,
                             ...
                             uint64_t /*end_lpn*/,
                             uint64_t /*size*/) {
    return IOStatus::NotSupported("AddInMeta is not supported for this FileSystem");
  }

  virtual uint64_t GetFreeBlocks(uint64_t /*pre_allocate_size*/,
                             int /*file_num*/,
                             uint64_t* /*start_lpns*/) {
    //return IOStatus::NotSupported("GetFreeBlocks is not supported for this FileSystem");
    return 0;
  }
  ...
  virtual IOStatus NewWritableFileFromFree(const std::string& /*fname*/,
                                ...
                                uint64_t /*start_lpn*/) {
    return IOStatus::NotSupported("NewWritableFileFromFree is not supported for this FileSystem");
  }

  virtual IOStatus ReturnFreeBlocks(uint64_t /*start_lpn*/,
                             uint64_t /*end_lpn*/) {
    return IOStatus::NotSupported("ReturnFreeBlocks is not supported for this FileSystem");
  }

  virtual void SetLevel(const std::string& /*fname*/,
                             int /*level*/) {
    fprintf(stderr, "SetLevel is not supported for this FileSystem");
    return;
  }
  std::atomic<uint64_t> g_memtable_num;
  std::atomic<uint64_t> g_memtable_size;
#endif
...
#if ROCKSDB_SPDK
  void SpdkMigration(std::string dbpath, std::string data_path) override {
    target_->SpdkMigration(dbpath, data_path);
  }

  IOStatus GetFileAddr(const std::string& fname,
                       uint64_t* start_lpn,
                       uint64_t* end_lpn) override {
    return target_->GetFileAddr(fname, start_lpn, end_lpn);
  }

  IOStatus AddInMeta(const std::string& fname,
                     uint64_t start_lpn,
                     uint64_t end_lpn,
                     uint64_t size) override {
    return target_->AddInMeta(fname, start_lpn, end_lpn, size);
  }

  uint64_t GetFreeBlocks(uint64_t pre_allocate_size,
                         int file_num,
                         uint64_t* start_lpns) override {
    return target_->GetFreeBlocks(pre_allocate_size, file_num, start_lpns);
  }

  IOStatus NewWritableFileFromFree(const std::string& fname,
                                std::unique_ptr<FSWritableFile>* result,
                                uint64_t pre_allocate_size,
                                uint64_t start_lpn) override {
    return target_->NewWritableFileFromFree(fname, result, pre_allocate_size, start_lpn);
  }

  IOStatus ReturnFreeBlocks(uint64_t start_lpn,
                         uint64_t end_lpn) override {
    return target_->ReturnFreeBlocks(start_lpn, end_lpn);
  }

  void SetLevel(const std::string& fname,
                             int level) override {
    target_->SetLevel(fname, level);
    return;
  }
#endif
...
FileSystem* NewSpdkEnv(const std::shared_ptr<FileSystem>& base_fs, Env* base_env, std::string pcie_addr, int open_mod);
#endif
