...
class VersionBuilder::Rep {
 ...
 public:
  ...
  void UnrefFile(FileMetaData* f) {
    ...
    if (f->refs <= 0) {
      if (f->table_reader_handle) {
        ...
        fprintf(stderr, "UnrefFile call ReleaseHandle: %lu\n", f->fd.GetNumber());
      ...
      for (size_t i = 1; i < level_files.size(); i++) {
        ...
        if (level == 0) {
          ...
          if (!level_zero_cmp_(f1, f2)) {
            fprintf(stderr, "CheckConsistencyDetails failed!!! 111\n");
          ...
          if (f2->fd.smallest_seqno == f2->fd.largest_seqno) {
            ...
            if (!(external_file_seqno < f1->fd.largest_seqno ||
              ...
              fprintf(stderr, "CheckConsistencyDetails failed!!! 222\n");
            ...
            fprintf(stderr, "CheckConsistencyDetails failed!!! 333 f1: %lu, f1 seq: %lu, f2: %lu, f2 seq: %lu\n", f1->fd.GetNumber(), f1->fd.smallest_seqno, f2->fd.GetNumber(), f2->fd.smallest_seqno);
            return Status::Corruption(
          ...
          if (!level_nonzero_cmp_(f1, f2)) {
            fprintf(stderr, "CheckConsistencyDetails failed!!! 555\n");
            return Status::Corruption(
          ...
          if (vstorage->InternalComparator()->Compare(f1->largest,
                                                      f2->smallest) >= 0) {
            fprintf(stderr, "CheckConsistencyDetails failed!!! 666\n");
            fprintf(stderr, "level: %d, file: %s, largest key: %s, vs. file: %s, smallest key: %s, %d, %d\n", level, NumberToString(f1->fd.GetNumber()).c_str(), (f1->largest).DebugString(true).c_str(), NumberToString(f2->fd.GetNumber()).c_str(), (f2->smallest).DebugString(true).c_str(), vstorage->InternalComparator()->Compare(f1->largest,f2->smallest) >= 0, vstorage->InternalComparator()->Compare(f1->smallest,f2->largest) <= 0);
            return Status::Corruption(
                "L" + NumberToString(level) +
  ...
  Status Apply(VersionEdit* edit) {
    {
      ...
      if (!s.ok()) {
        fprintf(stderr, "CheckConsistency failed!!! 333\n");
        return s;
      }
    }
  ...
  Status SaveTo(VersionStorageInfo* vstorage) {
    ...
    if (!s.ok()) {
      fprintf(stderr, "CheckConsistency failed!!! 111\n");
      return s;
    }

    s = CheckConsistency(vstorage);
    if (!s.ok()) {
      fprintf(stderr, "CheckConsistency failed!!! 222\n");
      return s;
    }
    ...
    s = CheckConsistency(vstorage);
    if (!s.ok()) {
      fprintf(stderr, "CheckConsistency failed!!! 444\n");
    }
    return s;
  }
...
#if LEMMA_L02L0
    for (auto& file_meta_pair : levels_[0].added_files) {
      auto* file_meta = file_meta_pair.second;
      if (!file_meta->is_wal) {
        // If the file has been opened before, just skip it.
        if (!file_meta->table_reader_handle) {
          files_meta.emplace_back(file_meta, 0);
