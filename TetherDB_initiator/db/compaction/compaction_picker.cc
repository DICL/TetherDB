...
bool FindIntraL0Compaction(const std::vector<FileMetaData*>& level_files,
  ...
  int64_t end = (int64_t)level_files.size() - 1;
  for (; end > 0; end--) {
    if (!level_files[end]->being_compacted) {
      break;
    }
  }
  size_t compact_bytes = static_cast<size_t>(level_files[end]->fd.file_size);
  uint64_t compensated_compact_bytes =
      level_files[end]->compensated_file_size;
  ...
  for (limit = end - 1; limit >= 0; limit--) {
    if(level_files[limit]->being_compacted || level_files[limit]->fd.largest_seqno > earliest_mem_seqno)
      break;
    ...
    new_compact_bytes_per_del_file = compact_bytes / (end - limit);
