...
const std::vector<FileMetaData*>& Compaction::LevelFiles(int level) const{
  return input_vstorage_->LevelFiles(level);
}
#endif
...
Compaction::Compaction(VersionStorageInfo* vstorage,
                       ...
                       CompactionReason _compaction_reason, size_t _total_len, int _base_index)
    : input_vstorage_(vstorage),
      ...
      compaction_reason_(_compaction_reason), total_len_(_total_len), base_index_(_base_index) {
...
bool Compaction::IsTrivialMove() const {
  ...
  for (const auto& file : inputs_.front().files) {
    ...
    if (compaction_size > max_compaction_bytes_) {
      fprintf(stderr, "not trivial because of ?? 111\n");
      return false;
    }
    ...
    if (partitioner.get() != nullptr) {
      if (!partitioner->CanDoTrivialMove(file->smallest.user_key(),
        ...
        fprintf(stderr, "not trivial because of ?? 222\n");
        return false;
      }
    }
  }
...
#if REMOTE_COMPACTION
void Compaction::AddInputDeletions(VersionEdit* out_edit, compaction_data::LocalCompactionRequest* local_compaction_result) {
  for (size_t which = 0; which < num_input_levels(); which++) {
    for (size_t i = 0; i < inputs_[which].size(); i++) {
      out_edit->DeleteFile(level(which), inputs_[which][i]->fd.GetNumber());
      local_compaction_result->add_deleted(inputs_[which][i]->fd.GetNumber());
      //local_compaction_result->add_deleted_level(level(which) == 0);
    }
  }
}

void Compaction::AddInputDeletions(compaction_data::LocalCompactionRequest* local_compaction_result) {
  for (size_t which = 0; which < num_input_levels(); which++) {
    for (size_t i = 0; i < inputs_[which].size(); i++) {
      local_compaction_result->add_deleted(inputs_[which][i]->fd.GetNumber());
      //local_compaction_result->add_deleted_level(level(which) == 0);
