...
#if DEBUG_PRINT
  fprintf(stderr, "!!!! GenSubcompaction : %d\n", c->max_subcompactions());//lemma
#endif
...
Status CompactionJob::Run() {
#if DEBUG_PRINT 
  fprintf(stderr, "\n!!!!! CompactionJob::Run called 777777777777777777777777777777\n"); //lemma
  clock_t start, finish;
  start = clock();
#endif

#if REMOTE_COMPACTION
  compaction_data::LocalCompactionRequest local_compaction_result;
  compact_->compaction->AddInputDeletions(&local_compaction_result);
  local_compaction_result.set_level(compact_->compaction->output_level());
  db_options_.greeter->LocalCompactionResult(local_compaction_result);
#endif
  ...
  size_t L0_files_count = compact_->compaction->start_level() == 0 ? compact_->compaction->num_input_files(0) : 0;
  size_t total_files_count = 0;
  for(size_t i = 0; i<compact_->compaction->num_input_levels(); i++)
    total_files_count += compact_->compaction->num_input_files(i);
  fprintf(stderr, "compaction done! leve_0_files:%lu, total_files:%lu, time: %lf\n", L0_files_count, total_files_count, duration);
#endif
  return status;
}
...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {//lemmalemma
  assert(sub_compact);
  ...
  read_options.is_compaction = true;
#endif
...
//#if REMOTE_COMPACTION
  //compaction->AddInputDeletions(edit, &local_compaction_result_);
//#else
  compaction->AddInputDeletions(edit);
//#endif

  //fprintf(stderr, "local compaction finished11111\n");
  for (const auto& sub_compact : compact_->sub_compact_states) {
    for (const auto& out : sub_compact.outputs) {
      edit->AddFile(compaction->output_level(), out.meta);
#if REMOTE_COMPACTION
      local_compaction_result_.add_smallest(out.meta.smallest.Encode().data());
      local_compaction_result_.add_largest(out.meta.largest.Encode().data());
      local_compaction_result_.add_file_size(out.meta.fd.file_size);
      local_compaction_result_.add_num_entries(out.meta.num_entries);
      local_compaction_result_.add_packed_number_and_path_id(out.meta.fd.packed_number_and_path_id);
      local_compaction_result_.add_largest_seqno(out.meta.fd.largest_seqno);
      local_compaction_result_.add_smallest_seqno(out.meta.fd.smallest_seqno);
#if ROCKSDB_SPDK
      std::string fname = TableFileName(compaction->immutable_cf_options()->cf_paths, out.meta.fd.GetNumber(), out.meta.fd.GetPathId());
      uint64_t start_lpn = 0;
      uint64_t end_lpn = 0;
      db_options_.spdk_fs->GetFileAddr(fname, &start_lpn, &end_lpn);
      local_compaction_result_.add_start_lpn(start_lpn);
      local_compaction_result_.add_end_lpn(end_lpn);
#endif
#endif
    }
#if REMOTE_COMPACTION
    local_compaction_result_.set_level(compaction->output_level());
#endif
  ...
  //fprintf(stderr, "local compaction finished2222\n");
#if REMOTE_COMPACTION
  db_options_.greeter->LocalCompactionResult(local_compaction_result_);
#endif

#if LEMMA_L0
  if(compaction->level() == 0) {
    return compaction->column_family_data()->imm()->TryInstallL0CompactionResults(
                compaction->column_family_data(), mutable_cf_options, edit, db_mutex_,
                db_directory_, compaction, versions_);
  } else {
#endif
...
Status CompactionJob::OpenCompactionOutputFile(
  ...
  Status s;
#if ROCKSDB_SPDK
  FileOptions file_options_tmp = file_options_;//lemma
  ...
  db_options_.spdk_fs->SetLevel(fname, sub_compact->compaction->output_level());//lemma
#else
  IOStatus io_s =
