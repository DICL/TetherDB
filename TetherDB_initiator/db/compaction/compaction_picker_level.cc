...
void LevelCompactionBuilder::SetupInitialFiles() {
      ...
      if (PickFileToCompact()) {
        ...
        if (start_level_ == 0) {
          ...
          if (PickIntraL0Compaction()) {
            ...
            compaction_reason_ = CompactionReason::kLevelL0FilesNum;
            break;
          }//lemmalemma. disable L0->L0 compaction
#endif
#if LEMMA_L02L0
          if (PickIntraL0Compaction()) {
            output_level_ = 0;
  ...
  if (!start_level_inputs_.empty()) {
    fprintf(stderr, "!!!!!!!!!!!!!!!!!!!!! shit shit FilesMarkedForCompaction\n");
  ...
  if (!start_level_inputs_.empty()) {
    fprintf(stderr, "!!!!!!!!!!!!!!!!!!!!! shit shit BottommostFiles\n");
  ...
  if (!start_level_inputs_.empty()) {
    fprintf(stderr, "!!!!!!!!!!!!!!!!!!!!! shit shit PeriodicCompaction\n");
...
Compaction* LevelCompactionBuilder::GetCompaction() {
#if DEBUG_PRINT
  fprintf(stderr, "\n!!! LevelCompactionBuilder::GetCompaction() called\n");//lemma
  for(CompactionInputFiles i : compaction_inputs_){
    for(FileMetaData* f : i.files){
      fprintf(stderr, "GetCompaction(): %d %lu %d\n", i.level, f->fd.GetNumber(), f->being_compacted);
    }
  }
#endif
  auto c = new Compaction(
      ...
      start_level_score_, false /* deletion_compaction */, compaction_reason_, total_len_, base_index_);
...
#if DEBUG_PRINT
  fprintf(stderr, "\n!!! LevelCompactionPicker::PickCompaction(~~~~~)  called\n");//lemma
#endif
