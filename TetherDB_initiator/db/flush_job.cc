...
Status FlushJob::WriteLevel0Table() {
...

  {
    ...
    {
#if DEBUG_PRINT
      fprintf(stderr, "!!!! flush started, memtables num : %lu, filenum: %lu\n", memtables.size(), meta_.fd.GetNumber());
#endif
  ...
  if (s.ok() && has_output) {
    ...
    //fprintf(stderr, "%lu, %s, %s, %lu, %lu\n", meta_.fd.GetNumber(), meta_.smallest.Encode().ToString().c_str(), meta_.largest.Encode().ToString().c_str(), meta_.fd.smallest_seqno, meta_.fd.largest_seqno);
