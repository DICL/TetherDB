...
#if LEMMA_L0
  uint64_t log_offset = wb->log_offset;
  uint64_t last_size = 0;
  uint64_t last_block_offset = (log_offset / (uint64_t)log::kBlockSize + 1) * (uint64_t)log::kBlockSize;
#endif
...
#if LEMMA_L0
        s = handler->PutCFWithOffset(column_family, key, value, log_offset);
        log_offset += last_size - input.size();
        if((log_offset >= last_block_offset) && input.size() > 0) {
          log_offset += log::kHeaderSize;
          last_block_offset += log::kBlockSize;
        }
#else
...
class MemTableInserter : public WriteBatch::Handler {
 ...
 public:
  ...
  const ColumnFamilyMemTables* CFMEMS() { return cf_mems_; }////////////////////////////////////////////////////////should be deleted lemmalemmalemmalemma
  ...
  Status PutCFImpl(uint32_t column_family_id, const Slice& key,
    ...
    if (!moptions->inplace_update_support) {
      ...
      assert(!concurrent_memtable_writes_);
fprintf(stderr, "PutCFImpl 444\n");
      ret_status = mem->Update(sequence_, key, value, kv_prot_info);
    } else {
      assert(!concurrent_memtable_writes_);
fprintf(stderr, "PutCFImpl 555\n");
...
#if LEMMA_L0
  Status PutCFWithOffset(uint32_t column_family_id, const Slice& key,
               const Slice& value, uint64_t log_offset) override {
    const auto* kv_prot_info = NextProtectionInfo();
    if (kv_prot_info != nullptr) {
      // Memtable needs seqno, doesn't need CF ID
      auto mem_kv_prot_info =
          kv_prot_info->StripC(column_family_id).ProtectS(sequence_);
      return PutCFImpl(column_family_id, key, value, kTypeValue,
                       &mem_kv_prot_info, log_offset);
    }
    return PutCFImpl(column_family_id, key, value, kTypeValue,
                     nullptr /* kv_prot_info */, log_offset);
  }
#endif
