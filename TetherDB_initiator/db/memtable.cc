...
MemTable::~MemTable() {
  ...
  spdk_fs_->g_memtable_size -= ApproximateMemoryUsage();
  fprintf(stderr, "~MemTable: %lu, total num: %lu, total size: %lu\n", logfile_number_, spdk_fs_->g_memtable_num.load(), spdk_fs_->g_memtable_size.load());
#endif
...
#if LEMMA_L0
  uint64_t log_offset() const override {
    return DecodeFixed64(value().data() + value().size());
    /*assert(Valid());
    Slice key_slice = GetLengthPrefixedSlice(iter_->key());
    Slice val_slice = GetLengthPrefixedSlice(key_slice.data() + key_slice.size());
    return DecodeFixed64(val_slice.data() + val_slice.size());*/
  }

  void set_index(uint64_t index) {
    //fprintf(stderr, "before key: %s\n", key().data());
    //fprintf(stderr, "before value: %s\n", value().data());
    //fprintf(stderr, "before offset: %lu\n", log_offset());
    EncodeFixed64(iter_->mutable_key() + (value().data() - iter_->key()) + value().size() + 8, index);
    //fprintf(stderr, "after key: %s\n", key().data());
    //fprintf(stderr, "after value: %s\n", value().data());
    //fprintf(stderr, "after offset: %lu\n", log_offset());
    //fprintf(stderr, "after index: %lu\n", DecodeFixed64(iter_->mutable_key() + (value().data() - iter_->key()) + value().size() + 8));
  }

  uint64_t get_index() const override {
    return DecodeFixed64(value().data() + value().size() + 8);
  }
#endif
...
InternalIterator* MemTable::NewIterator() {
  ...
  return new MemTableIterator(*this, ro, nullptr);
}
...
Status MemTable::Add(SequenceNumber s, ValueType type,
  ...
  //lemma_assert assert((unsigned)(p + val_size - buf) == (unsigned)encoded_len);
