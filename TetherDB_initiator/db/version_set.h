...
#if LEMMA_L0
  int l0_delay_trigger_count() const { return l0_delay_trigger_count_ / 2; }
#else
...
#if REMOTE_RANGE
  Slice GetSmallestUserKey() { return smallest_.user_key(); }
  Slice GetLargestUserKey() { return largest_.user_key(); }
  bool DidSet() { return smallest_.size() == 0 ? false : true; }
#endif
 private:
...
class Version {
 public:
  ...
  void GetTestLemma(const ReadOptions& read_options, const LookupKey& k, PinnableSlice* value,
...
class VersionSet {
 public:
  ...
  uint64_t NewFileNumber(size_t num) { return next_file_number_.fetch_add(num); }
#endif
