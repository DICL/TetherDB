...
#if DEBUG_PRINT
  fprintf(stderr, "\n!!! ColumFamilyData created. data id: %d, name: %s, kDummyColumnFamilyDataId: %d \n", id_, name.c_str(), kDummyColumnFamilyDataId);//lemma
#endif
  ...
  if (_dummy_versions != nullptr) {
    ...
    if (ioptions_.compaction_style == kCompactionStyleLevel) {
...
#if DEBUG_PRINT
      fprintf(stderr, "\n!!! column_family.cc call LevelCompactionPicker\n");//lemma
#endif
#ifndef ROCKSDB_LITE
...
#if DEBUG_PRINT
      fprintf(stderr, "\n!!! cloumn_family.cc call UniversalCompactionPicker\n");//lemma
#endif
    } else if (ioptions_.compaction_style == kCompactionStyleFIFO) {
      compaction_picker_.reset(
          new FIFOCompactionPicker(ioptions_, &internal_comparator_));
#if DEBUG_PRINT
      fprintf(stderr, "\n!!! cloumn_family.cc call FIFOCompactionPicker\n");//lemma
#endif
    } else if (ioptions_.compaction_style == kCompactionStyleNone) {
      compaction_picker_.reset(new NullCompactionPicker(
          ioptions_, &internal_comparator_));
#if DEBUG_PRINT
      fprintf(stderr, "\n!!! cloumn_family.cc call NullCompactionPicker\n");//lemma
#endif
      ...
      compaction_picker_.reset(
          new LevelCompactionPicker(ioptions_, &internal_comparator_));
#if DEBUG_PRINT
      fprintf(stderr, "\n!!! column_family.cc call LevelCompactionPicker2\n");//lemma
#endif
    }
...
ColumnFamilyData::GetWriteStallConditionAndCause(
  ...
  //return {WriteStallCondition::kNormal, WriteStallCause::kNone};//lemma
  ...
  if (current_ != nullptr) {
    ...
    for(int level = 0; level < vstorage->num_levels(); level++){
      ...
      for(auto& f : vstorage->LevelFiles(level)){
        fprintf(stderr, "%lu ", f->fd.GetNumber());
      }
      fprintf(stderr, "\n");
    }
#endif
    ...
    if (write_stall_condition == WriteStallCondition::kStopped &&
      ...
      if (vstorage->l0_delay_trigger_count() >=
        ...
        write_controller_token_.reset();//lemmalemma
      }
...
Compaction* ColumnFamilyData::PickCompaction(
  ...
  SequenceNumber earliest_mem_seqno =
      std::min(mem_->GetEarliestSequenceNumber(),
               imm_.GetCurrentEarliestSequenceNumber());
#else
  SequenceNumber earliest_mem_seqno =
      std::min(mem_->GetEarliestSequenceNumber(),
...
#if DEBUG_PRINT
  fprintf(stderr, "\n!!! ColumnFamilySet::CreateColumnFamily called\n");//lemma
#endif
