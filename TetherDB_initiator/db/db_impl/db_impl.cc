...
DBImpl::DBImpl(const DBOptions& options, const std::string& dbname,
    ...
    : dbname_(dbname),
      ...
      greeter_(options.greeter),//lemma
#endif
...
#if REMOTE_COMPACTION
      num_running_remote_compactions_(0),
      max_running_remote_compactions_(0),
#endif
...
#if DEBUG_PRINT
  fprintf(stderr, "!@@@@@@@@@@@@@@@@@@@@@@ db_impl create 1111111111 %s @@@@@@@@@@@@@@@@@@@@@@@!!!!!!!!!\n", dbname.c_str());//lemma
#endif
  versions_.reset(new VersionSet(dbname_, &immutable_db_options_, file_options_,
                                 ...
                                 io_tracer_));
#if DEBUG_PRINT
  fprintf(stderr, "!@@@@@@@@@@@@@@@@@@@@@@ db_impl create 2222222222 @@@@@@@@@@@@@@@@@@@@@@@!!!!!!!!!\n");//lemma
#endif
...
Status DBImpl::ResumeImpl(DBRecoverContext context) {
  ...
  if (s.ok()) {
    ...
    fprintf(stderr, "MaybeScheduleFlushOrCompaction 111 \n");//lemma
...
void DBImpl::MaybeIgnoreError(Status* s) const {
  if (s->ok() || immutable_db_options_.paranoid_checks) {
    ...
    ROCKS_LOG_WARN(immutable_db_options_.info_log, "Ignoring error %s",
                   s->ToString().c_str());
    fprintf(stderr, "MaybeIgnoreError\n");
    *s = Status::OK();
  }
}
...
#if DEBUG_PRINT
        fprintf(stderr, "\n\n!!!!!!!!!!! DBImpl::SetDBOptions call MaybeScheduleFlushOrCompaction\n");//lemma
#endif
        fprintf(stderr, "MaybeScheduleFlushOrCompaction 222 \n");//lemma
        MaybeScheduleFlushOrCompaction();
      }
...
Status DBImpl::Get(const ReadOptions& read_options,
  ...
  SuperVersion* sv = GetAndRefSuperVersion(cfd);
    ...
    auto prefix_extractor = sv->mutable_cf_options.prefix_extractor;
#endif

  Status s = GetImpl(read_options, key, get_impl_options);
#if LEMMA_L0 && DEBUG_PRINT
  if(!s.ok()) {
    fprintf(stderr, "Get Fail!! %lu %s %lu\n", sv->imm->GetTotalNumEntries(), key.data(), key.size());
    VersionStorageInfo* tmp_vsi = sv->current->storage_info();
    int tmp_level = tmp_vsi->num_non_empty_levels();
    /*for(MemTable* mem : sv->imm->GetList()){
      ReadOptions ro;
      ...
      std::string tmp_ptr;
      while(iter->Valid()) {
        std::string tmp(iter->key().data(), iter->key().size() - kNumInternalBytes);
        //Slice tmpppp(tmpp);         
        //if(cfd->user_comparator()->Compare(tmpppp, key) == 0){
        if(tmp.compare(key.ToString()) == 0){
          fprintf(stderr, "found????%lu\n", mem->GetLogNumber());
          break;
        //} else if(cfd->user_comparator()->Compare(tmpppp, key) < 0) {
        } else if(tmp.compare(key.ToString()) < 0) {
           tmp_ptr = tmp;
        } else {
          if(kk) {
            kk = false;
            fprintf(stderr, "not found %s, %s, %lu\n", tmp_ptr.c_str(), tmp.c_str(), mem->GetLogNumber());
          }
        }
        k++;
        iter->Next();
    ...
    for(int i = 1; i<tmp_level; i++){
      for(auto f : tmp_vsi->LevelFiles(i)) {
        fprintf(stderr, "files %d %lu %s %s\n", i, f->fd.GetNumber(), f->smallest.user_key().ToString().c_str(), f->largest.user_key().ToString().c_str());
      }
    }
    sv->current->GetTestLemma(
    ...
    sv->current->Get(
        ...
        get_impl_options.get_value ? get_impl_options.is_blob_index : nullptr,
        get_impl_options.get_value);
    fprintf(stderr, "status.ok()?? %d\n", s.ok());
///////////////////////////////////////////////////////////
    for(int i = 1; i<tmp_level; i++){
      for(auto f : tmp_vsi->LevelFiles(i)) {
        fprintf(stderr, "files 22 %d %lu %s %s\n", i, f->fd.GetNumber(), f->smallest.user_key().ToString().c_str(), f->largest.user_key().ToString().c_str());
        std::string small = f->smallest.user_key().ToString();
        std::string large = f->largest.user_key().ToString();
        if(small.compare(key.ToString()) < 0 && large.compare(key.ToString()) > 0) {  
          InternalIterator* iter = cfd->table_cache()->NewIterator(
            read_options, file_options_, cfd->internal_comparator(),
            *f, /*range_del_agg=*/nullptr,
            prefix_extractor.get(),
            /*table_reader_ptr=*/nullptr,
            cfd->internal_stats()->GetFileReadHist(4),
            TableReaderCaller::kCompactionRefill, /*arena=*/nullptr,
            /*skip_filters=*/false, 4,
            MaxFileSizeForL0MetaPin(
                sv->mutable_cf_options),
            /*smallest_compaction_key=*/nullptr,
            /*largest_compaction_key=*/nullptr,
            /*allow_unprepared_value=*/false);
          iter->SeekToFirst();
          fprintf(stderr, "iter->SeekToFirst() good!!\n");
          int k = 0;
          bool kk = true;
          std::string tmp_ptr;
          while(iter->Valid()) {
            ...
            if(tmp.compare(key.ToString()) == 0){
             ...
             //} else if(cfd->user_comparator()->Compare(tmpppp, key) < 0) {
              ...
              if(kk) {
                kk = false;
                fprintf(stderr, "not found %s, %s, %lu\n", tmp_ptr.c_str(), tmp.c_str(), f->fd.GetNumber());
              }
            }
            k++;
            iter->Next();
          }
          fprintf(stderr, "%lu %d %s %s\n", f->fd.GetNumber(), k, small.c_str(), large.c_str());
        }
      }
    }
    FileMetaData* i = new FileMetaData();
    i->log_sst->MakeLogSstMeta();
  }
#endif
  return s;
}
...
Status DBImpl::GetImpl(const ReadOptions& read_options, const Slice& key,
  ...
  if (!skip_memtable) {
    ...
    if (!done && !s.ok() && !s.IsMergeInProgress()) {
      fprintf(stderr, "yayayayayayayayayayyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy 11\n");
  ...
  if (!done) {
    ...
    //if(s.ok() == 0)
      //fprintf(stderr, "yayayayayayayayayayyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy 22 skip_memtable: %d\n", skip_memtable);
  }
      ...
      if (get_impl_options.get_value) {
        ...
        *get_impl_options.number_of_operands =
        ...
        if (*get_impl_options.number_of_operands >
          ...
          fprintf(stderr, "yayayayayayayayayayyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy\n");
          s = Status::Incomplete(
  ...
  //if (!s.ok())
    //fprintf(stderr, "not found! imm: %lu\n", sv->imm->GetList().size());
  return s;
}
    ...
    if (oldest_snapshot > bottommost_files_mark_threshold_) {
      ...
      for (auto* cfd : *versions_->GetColumnFamilySet()) {
        ...
        if (!cfd->current()
          ...
          SchedulePendingCompaction(cfd);
#if DEBUG_PRINT
          fprintf(stderr, "\n\n!!!!!!!!!!!!!! DBImpl::ReleaseSnapshot call MaybeScheduleFlushOrCompaction\n");//lemma
#endif
          fprintf(stderr, "MaybeScheduleFlushOrCompaction 333 \n");//lemma
          MaybeScheduleFlushOrCompaction();
...
#if ROCKSDB_SPDK
  if (options.spdk_fs != nullptr)
    options.spdk_fs->DeleteDir(dbname, IOOptions(), nullptr);
#endif
