...
#include "lemma/remote_compaction_job.h" //lemmalemmalemma
#include "lemma/remote_compaction.h" //lemmalemmalemma
...
Status DBImpl::FlushMemTableToOutputFile(
  ...
  cfd->imm()->PickMemtablesToFlush(port::kMaxUint64, &mems);
  // 1. make logsst
  // 2. add logsst to vstorageinfo
  // 3, mark mamtables as flushed and skip when pick memtable -> nothing to do with this
  SyncClosedLogs(job_context);
  mutex_.Unlock();
  VersionEdit* edit = new VersionEdit();
#if REMOTE_COMPACTION
#if ROCKSDB_SPDK
  FileSystem* spdk_fs = cfd->ioptions()->spdk_fs;//lemma
#endif
#endif
  for(MemTable* m : mems) {
    LogSst* tmp_log_sst = new LogSst(m, immutable_db_options_);
    FileMetaData* tmp_meta = tmp_log_sst->MakeLogSstMeta();
    edit->AddFile(0, *tmp_meta);
#if REMOTE_COMPACTION
#if ROCKSDB_SPDK
    std::string fname = LogFileName(immutable_db_options_.wal_dir, m->GetLogNumber());
    uint64_t start_lpn = 0;
    uint64_t end_lpn = 0;
    spdk_fs->GetFileAddr(fname, &start_lpn, &end_lpn);
    immutable_db_options_.greeter->Caching(tmp_meta, start_lpn, end_lpn);//lemma
#else
    immutable_db_options_.greeter->Caching(tmp_meta);//lemma
#endif
#endif
    delete tmp_meta;
  }
  ...
  versions_->LogAndApply(cfd, mutable_cf_options, edit, &mutex_, directories_.GetDbDir());
  delete edit;
...
Status DBImpl::AtomicFlushMemTablesToOutputFiles(
  ...
  for (int i = 0; i < num_cfs; ++i) {
    ...
    fprintf(stderr, "new FlushJob!!! 22\n");
...
fprintf(stderr, "FindObsoleteFiles 44\n");
...
#if DEBUG_PRINT
  fprintf(stderr, "\n!!! DBImpl::RunManualCompaction called\n");//lemma
#endif
...
#if DEBUG_PRINT
  fprintf(stderr, "\n!!! DBImpl::BGWorkCompaction called, prepicked: %d \n", prepicked_compaction == nullptr);//lemma
#endif
...
#if DEBUG_PRINT
  fprintf(stderr, "\n!!! DBImpl::BGWorkBottomCompaction called \n");//lemma
#endif
...
#if REMOTE_COMPACTION
    //if(c->start_level() == 0 || (c->start_level() < remote_level && num_running_remote_compactions_ < 5)) {
    //if(c->start_level() < remote_level) {
#if REMOTE_RANGE
    bool remote_bool = (c->start_level() == 0 || (c->start_level() < 2 && greeter_->CheckRange(c.get())));
#else
    int remote_level = 2;
    bool remote_bool = (c->start_level() < remote_level);
#endif
    if(remote_bool) {
      num_running_remote_compactions_++;
      mutex_.Unlock();
#if DEBUG_PRINT
      fprintf(stderr, "compaction but check stall %d %d\n", write_controller_.IsStopped(), write_controller_.NeedsDelay());
      for(size_t i = 0; i < c->num_input_levels(); i++){
        fprintf(stderr, "compaction input level: %d, num: %lu\n", c->level(i), c->num_input_files(i));
      }
#endif
      ...
      std::vector<FileMetaData> remote_result = greeter_->SayHello(c.get(), job_context->job_id,
                              shutting_down_, preserve_deletes_seqnum_.load(), earliest_write_conflict_snapshot,
                              thread_pri, is_manual ? &manual_compaction_paused_ : nullptr, db_id_, db_session_id_,
                              c->column_family_data()->GetFullHistoryTsLow());
#if DEBUG_PRINT
      if(c->start_level() == 0 && c->output_level() == 0) {
        fprintf(stderr, "0 to 0!! \n");
       for(auto tt : *(c->inputs(0)))
          fprintf(stderr, "input: %lu\n", tt->fd.GetNumber());
        for(auto tt : remote_result)
          fprintf(stderr, "output: %lu\n", tt.fd.GetNumber());
        fprintf(stderr, "\n");
      }
#endif
      if(remote_result.size() > 0){
        VersionEdit* const edit = c->edit();
        c->AddInputDeletions(edit);
        for(auto out : remote_result){
          edit->AddFile(output_level, out);
        }
        size_t total_input_num = 0;//lemma
        for(size_t tmp = 0; tmp < c->num_input_levels(); tmp++)//lemma
          total_input_num += c->num_input_files(tmp);//lemma
        mutex_.Lock();
        num_remote_c_++;
#if LEMMA_L0
        if(c->level() == 0) {
          status = c->column_family_data()->imm()->TryInstallL0CompactionResults(
                      c->column_family_data(), *c->mutable_cf_options(), edit, &mutex_,
                      directories_.GetDbDir(), c.get(), versions_.get());
        } else {
#endif
          status = versions_->LogAndApply(c->column_family_data(), *c->mutable_cf_options(), edit, &mutex_, directories_.GetDbDir());
#if LEMMA_L0
        }
#endif
        if (status.ok()) {
          //fprintf(stderr, "@@@@@@@@@@@@@ ++++++++ _____________ ----- reply: RPC good !!\n");
          InstallSuperVersionAndScheduleWork(c->column_family_data(),
                                           &job_context->superversion_contexts[0],
                                           *c->mutable_cf_options());
          *made_progress = true;
        } else {
          fprintf(stderr, "\n\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ ++++++++ _____________ ----- reply: RPC failed 11 !! %d\n\n", job_context->job_id);
        }
      } else {
        status = Status::Incomplete();
        //fprintf(stderr, "\n\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ ++++++++ _____________ ----- reply: RPC failed !! %d\n\n", job_context->job_id);
        mutex_.Lock();
      }
      num_running_remote_compactions_--;
    }
    //if (c->start_level() >= remote_level || !status.ok()) {
    if (!remote_bool || !status.ok()) {
      //if (!status.ok())
      ...
      assert(is_snapshot_supported_ || snapshots_.empty());
#if DEBUG_PRINT
      fprintf(stderr, "compaction inputs: ");
      if(c->start_level() == 0) {
        for(size_t kkk = 0; kkk < c->num_input_files(0); kkk++)
          fprintf(stderr, "%lu ", c->input(0, kkk)->fd.GetNumber());
        if (c->num_input_levels() > 1)
          for(size_t kkk = 0; kkk < c->num_input_files(1); kkk++)
            fprintf(stderr, "%lu ", c->input(1, kkk)->fd.GetNumber());
      }
      fprintf(stderr, "\n");
#endif

      CompactionJob compaction_job( //lemmalemmalemma
          job_context->job_id, c.get(), immutable_db_options_,
          file_options_for_compaction_, versions_.get(), &shutting_down_,
          preserve_deletes_seqnum_.load(), log_buffer, directories_.GetDbDir(),
          GetDataDir(c->column_family_data(), c->output_path_id()),
      ...
      //fprintf(stderr, "@@@@@@@@@@@@@ ++++++++ _____________ ----- not RPC !!\n");
      *made_progress = true;
      TEST_SYNC_POINT_CALLBACK("DBImpl::BackgroundCompaction:AfterCompaction",
                               c->column_family_data());
#if REMOTE_COMPACTION
...
void DBImpl::InstallSuperVersionAndScheduleWork(
  ...
  if (UNLIKELY(sv_context->new_superversion == nullptr)) {
    fprintf(stderr, "unlikely to step in!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
