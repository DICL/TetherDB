...
void DBImpl::FindObsoleteFiles(JobContext* job_context, bool force,
...
#if LEMMA_L0
  job_context->log_number = 0;
  job_context->prev_log_number = 0;
  //job_context->log_number = logs_.front().number;
  //job_context->prev_log_number = logs_.front().number;
#else
  ...
  if (!alive_log_files_.empty() && !logs_.empty()) {
    auto log_files_it = alive_log_files_.begin();
    size_t num_alive_log_files = alive_log_files_.size();
    uint64_t min = job_context->log_number;
    while (log_files_it != alive_log_files_.end()) {
      if(*(log_files_it->del)) {
        if (immutable_db_options_.recycle_log_file_num >
          ...
          ROCKS_LOG_INFO(immutable_db_options_.info_log,
                         ...
                         log_files_it->number);
          log_recycle_files_.push_back(log_files_it->number);
        } else {
          job_context->log_delete_files.push_back(log_files_it->number);
        }
        if (job_context->size_log_to_delete == 0) {
          job_context->prev_total_log_size = total_log_size_;
          job_context->num_alive_log_files = num_alive_log_files;
        }
        job_context->size_log_to_delete += log_files_it->size;
        total_log_size_ -= log_files_it->size;
        if(min < log_files_it->number) min = log_files_it->number;
        if (two_write_queues_) {
          log_write_mutex_.Lock();
        }
        log_files_it = alive_log_files_.erase(log_files_it);
        if (two_write_queues_) {
          log_write_mutex_.Unlock();
        }
      } else ++log_files_it;
    }
/*    auto logs_it = logs_.begin();
    while (logs_it != logs_.end()) {
      if(*(logs_it->del)) {
        if (logs_it->getting_synced) {
          log_sync_cv_.Wait();
           // logs_ could have changed while we were waiting.
          continue;
        }
        fprintf(stderr, "logs to free +++++++++++++++++++++++++++++++++++++++++++++ %lu\n", logs_it->number);
        logs_to_free_.push_back(logs_it->ReleaseWriter());
//        if(min < logs_it->number) min = logs_it->number;
        {
          InstrumentedMutexLock wl(&log_write_mutex_);
          logs_it = logs_.erase(logs_it);
        }
      } else ++logs_it;
    }*/
    min++;
    job_context->log_number = min;
    job_context->prev_log_number = min;
  }
#else
...
#if DEBUG_PRINT
#if LEMMA_L0
        fprintf(stderr, "Purge!!! keep: %d, number: %lu, number >= state.log_number: %d, (number == state.prev_log_number): %d \n", keep, number, (number >= state.log_number), (number == state.prev_log_number));
#endif
#endif
        break;
