...
class DBImpl : public DB {
 public:
  ...
  virtual void PrintRemoteLocal() override {
    fprintf(stderr, "%d %d\n", num_remote_c_, num_local_c_);
  }
#endif
 protected:
...
#if LEMMA_L0
  IOStatus WriteToWAL(const WriteBatch& merged_batch, log::Writer* log_writer,
                      uint64_t* log_used, uint64_t* log_size, const WriteThread::WriteGroup& write_group);
#endif
...
#if REMOTE_COMPACTION
  int num_running_remote_compactions_;
  int max_running_remote_compactions_;
  int num_remote_c_ = 0;
  int num_local_c_ = 0;
#endif
