...
DBOptions SanitizeOptions(const std::string& dbname, const DBOptions& src) {
  ...
  if (result.max_open_files != -1) {
    ...
    TEST_SYNC_POINT_CALLBACK("SanitizeOptions::AfterChangeMaxOpenFiles", &result.max_open_files);
  }
  ...
  if (result.delayed_write_rate == 0) {
    ...
    if (result.delayed_write_rate == 0) {
      ...
      //result.delayed_write_rate = 64 * 1024 * 1024;
    }
  }
...
Status DBImpl::Recover(
  ...
  if (s.ok()) {
...
#if ROCKSDB_SPDK
      s = spdk_fs_->GetChildren(immutable_db_options_.wal_dir, IOOptions(), &files_in_wal_dir, nullptr);
#endif
    }
    if (s.IsNotFound()) {
    ...
    if (!wal_files.empty()) {
      ...
      if (!s.ok()) {
        fprintf(stderr, "no!!!!!!!!!!!!!!!!!!!!! RecoverLogFiles failed\n");
      ...
      Env::Priority thread_pri = Env::Priority::HIGH;
      for (auto cfd : *versions_->GetColumnFamilySet()) {
        JobContext job_context(next_job_id_.fetch_add(1), true);
        SuperVersionContext* superversion_context = new SuperVersionContext(true);
        FlushMemTableToOutputFile(cfd, *cfd->GetLatestMutableCFOptions(), nullptr, nullptr, superversion_context, snapshot_seqs, 0, nullptr, nullptr, thread_pri);
      }
#endif
    }
  }
  ...
  for (auto wal_number : wal_numbers) {
...
#if ROCKSDB_SPDK
      status = spdk_fs_->NewSequentialFile(fname,
...
#endif
      if (!status.ok()) {
        fprintf(stderr, "NewSequentialFile %s error\n", fname.c_str());
    ...
    while (!stop_replay_by_wal_filter &&
      ...
      if (record.size() < WriteBatchInternal::kHeader) {
        ...
        fprintf(stderr, "continue 44\n");
        continue;
      }
#if LEMMA_L0
      uint64_t last_off = reader.LastRecordOffset();
      uint64_t left = log::kBlockSize - (last_off % log::kBlockSize);
      if(left < log::kHeaderSize) {
        last_off = (last_off / log::kBlockSize + 1) * log::kBlockSize;
      }
      if(left <= 19 && left >= log::kHeaderSize) {
        last_off += 7;
      }
      batch.log_offset = last_off + 19;
#endif
      ...
      if (immutable_db_options_.wal_filter != nullptr) {
        ...
        switch (wal_processing_option) {
          ...
          case WalFilter::WalProcessingOption::kIgnoreCurrentRecord:
            // skip current record
            fprintf(stderr, "continue 55\n");
            continue;
          case WalFilter::WalProcessingOption::kStopReplay:
            // skip current record and stop replay
            stop_replay_by_wal_filter = true;
            fprintf(stderr, "continue 66\n");
            continue;
...
#if LEMMA_L0
    for (auto cfd : *versions_->GetColumnFamilySet()) {
      if (cfd->mem()->num_entries() > 0) {
        LogFileNumberSize log(wal_number);
#if ROCKSDB_SPDK
        Status s = spdk_fs_->GetFileSize(fname, IOOptions(), &log.size, nullptr);
#else
        Status s = env_->GetFileSize(fname, &log.size);
#endif
        if (!s.ok()) {
          break;
        }
        ...
        flushed = true;
        cfd->mem()->SetLogNumber(wal_number);
        //cfd->mem()->SetLog(nullptr, alive_log_files_.back().del);
        cfd->mem()->SetLog(alive_log_files_.back().del);
#if DEBUG_PRINT
        fprintf(stderr, "!!!!!!!!!!!!!!!!!! logsize: %lu, memsize: %lu, log_num: %lu, ref_count:%d\n", log.size, cfd->mem()->num_entries(), wal_number, cfd->mem()->RefCount());
#endif
        WriteContext context;
        cfd->imm()->Add(cfd->mem(), &context.memtables_to_free_);
        cfd->SetMemtable(cfd->ConstructNewMemtable(*cfd->GetLatestMutableCFOptions(), *next_sequence));
        cfd->mem()->Ref();
      }
    }
#endif
...
#if LEMMA_L0
          LogFileNumberSize log(max_wal_number);
          std::string fname = LogFileName(immutable_db_options_.wal_dir, max_wal_number);
#if ROCKSDB_SPDK
          Status s = spdk_fs_->GetFileSize(fname, IOOptions(), &log.size, nullptr);
#else
          Status s = env_->GetFileSize(fname, &log.size);
...
#if DEBUG_PRINT
          fprintf(stderr, "!!!!!!!!!!!!!!!!!! logsize: %lu, memsize: %lu, log_num: %lu\n", log.size, cfd->mem()->num_entries(), max_wal_number);
#endif
          total_log_size_ += log.size;
          alive_log_files_.push_back(log);
          flushed = true;
          cfd->mem()->SetLogNumber(max_wal_number);
          //cfd->mem()->SetLog(nullptr, alive_log_files_.back().del);
          cfd->mem()->SetLog(alive_log_files_.back().del);
          WriteContext context;
          cfd->imm()->Add(cfd->mem(), &context.memtables_to_free_);
...
Status DBImpl::RestoreAliveLogFiles(const std::vector<uint64_t>& wal_numbers) {
  fprintf(stderr, "RestoreAliveLogFiles kiiiiiiiiiiiiiiiiiiiiiiiiiiiiii %lu\n", wal_numbers[0]);
...
Status DB::Open(const DBOptions& db_options, const std::string& dbname,
  ...
  DBOptions db_options1 = db_options;//lemma
  fprintf(stderr, "DB::Open 22\n");
#if REMOTE_COMPACTION
  if (db_options.greeter == nullptr)
    db_options1.greeter = new GreeterClient("11.0.0.91:50052");// + std::to_string(2 + MY_ID));
#endif

#if ROCKSDB_SPDK
  if (db_options.spdk_fs == nullptr)
    //db_options1.SetSpdkEnv("trtype:PCIe adrfam:IPv4 traddr:0000:86:00.0");
    db_options1.SetSpdkEnv("trtype:RDMA adrfam:IPv4 traddr:11.0.0.91 trsvcid:4421 subnqn:nqn.2019-04.pos:subsystem1");
  //db_options1.SetSpdkEnv("trtype:RDMA adrfam:IPv4 traddr:10.0.0.90 trsvcid:4421 subnqn:nqn.2016-06.io.spdk:cnode1"); //"trtype:PCIe traddr:0000:86:00.0");//lemma  "trtype:RDMA adrfam:IPv4 traddr:10.0.0.11 trsvcid:4420 subnqn:nqn.2016-06.io.spdk:cnode1"
#endif
  ...
  *dbptr = nullptr;
    ...
    for (auto cfd : *impl->versions_->GetColumnFamilySet()) {
      ...
      for (int i = 0; i < vstorage->num_levels(); ++i) {
        for (auto f : vstorage->LevelFiles(i)) {
          std::string fname = TableFileName(cfd->ioptions()->cf_paths, f->fd.GetNumber(), f->fd.GetPathId());
          db_options.spdk_fs->SetLevel(fname, i);
        }
      }
    }
  }
  ...
  if (s.ok()) {
    for (auto cfd : *impl->versions_->GetColumnFamilySet()) {
      auto* vstorage = cfd->current()->storage_info();
      for (int i = 0; i < vstorage->num_levels(); ++i) {
        compaction_data::LocalCompactionRequest tmp_edit;
        for (auto f : vstorage->LevelFiles(i)) {
#if LEMMA_L0
          if (!f->is_wal) {
#endif
            tmp_edit.add_smallest(f->smallest.Encode().data());
            tmp_edit.add_largest(f->largest.Encode().data());
            tmp_edit.add_file_size(f->fd.file_size);
            tmp_edit.add_num_entries(f->num_entries);
            tmp_edit.add_packed_number_and_path_id(f->fd.packed_number_and_path_id);
            tmp_edit.add_largest_seqno(f->fd.largest_seqno);
            tmp_edit.add_smallest_seqno(f->fd.smallest_seqno);
#if ROCKSDB_SPDK
            std::string fname = TableFileName(cfd->ioptions()->cf_paths, f->fd.GetNumber(), f->fd.GetPathId());
            uint64_t start_lpn = 0;
            uint64_t end_lpn = 0;
            db_options.spdk_fs->GetFileAddr(fname, &start_lpn, &end_lpn);
            tmp_edit.add_start_lpn(start_lpn);
            tmp_edit.add_end_lpn(end_lpn);
#endif
#if LEMMA_L0
          }
#endif
        }
        tmp_edit.set_level(i);
        db_options.greeter->LocalCompactionResult(tmp_edit);
      }
    }
  }
#endif
  ...
  /*for (auto* h : *handles) {
    auto cfh = static_cast_with_check<ColumnFamilyHandleImpl>(h);
    auto cfd = cfh->cfd();
    SuperVersion* sv = GetAndRefSuperVersion(cfd);
    VersionStorageInfo* tmp_vsi = sv->current->storage_info();
    for (int i = 0; i < tmp_vsi->num_levels(); i++)
      fprintf(stderr, "%d\n", tmp_vsi->NumLevelFiles(i));
  }*/
  return s;
}
}  // namespace ROCKSDB_NAMESPACE
