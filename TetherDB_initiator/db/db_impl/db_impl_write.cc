...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
  ...
  if(w.log_used != 0)
    fprintf(stderr, "log_used is not 0 : %lu", w.log_used);
...
IOStatus DBImpl::WriteToWAL(const WriteBatch& merged_batch,
                            log::Writer* log_writer, uint64_t* log_used,
                            uint64_t* log_size, const WriteThread::WriteGroup& write_group) {
  assert(log_size != nullptr);
  Slice log_entry = WriteBatchInternal::Contents(&merged_batch);
  *log_size = log_entry.size();
  const bool needs_locking = manual_wal_flush_ && !two_write_queues_;
  ...
  size_t start_index, start_offset, end_index, end_offset;
  int header_size;
  IOStatus io_s = log_writer->AddRecord(log_entry, start_index, start_offset, end_index, end_offset, header_size);

  if (UNLIKELY(needs_locking)) {
    log_write_mutex_.Unlock();
  }

  start_offset += WriteBatchInternal::kHeader + header_size;
  int count = 0;
  if(start_offset >= log::kBlockSize) {
    start_offset = start_offset + header_size - log::kBlockSize;
    start_index++;
  }
  for (auto writer : write_group) {
    writer->batch->log_offset = start_index*log::kBlockSize + start_offset;
    size_t left_data_size = writer->batch->GetDataSize() - WriteBatchInternal::kHeader;
    while(left_data_size > 0) {
      if(log::kBlockSize > start_offset + left_data_size) {
        start_offset += left_data_size;
        break;
      } else {
        count++;
        left_data_size -= (log::kBlockSize - start_offset);
...
#if DEBUG_PRINT
  if(start_offset != end_offset || start_index != end_index)
    if(start_index != end_index+1 || start_offset != 7 || end_offset != 32768)
      fprintf(stderr, "!!!!!!!!!!!!!!!!!!!!!! %lu %lu %lu %lu %d\n", start_index, start_offset, end_index, end_offset, count);
#endif
...
IOStatus DBImpl::WriteToWAL(const WriteThread::WriteGroup& write_group,
  ...
  /*if(merged_batch->GetDataSize() != 153) {
  fprintf(stderr, "!!!!!!!!!");
  for(size_t i=0; i<merged_batch->GetDataSize(); i++){
    int tmp = merged_batch->Data()[i];
    if(33 <= tmp && tmp <= 126)
      fprintf(stderr, "%c", tmp);
    else
      fprintf(stderr, "|%d|", tmp);
  }
  fprintf(stderr, " || %lu \n", merged_batch->GetDataSize());
  }*/
  ...
  uint64_t log_size;
#if LEMMA_L0
  io_s = WriteToWAL(*merged_batch, log_writer, log_used, &log_size, write_group);
#else
...
Status DBImpl::ThrottleLowPriWritesIfNeeded(const WriteOptions& write_options,
                                            WriteBatch* my_batch) {
  fprintf(stderr, "ThrottleLowPriWritesIfNeeded !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \n");
...
Status DBImpl::SwitchMemtable(ColumnFamilyData* cfd, WriteContext* context) {
  ...
  if (s.ok() && creating_new_log) {
    ...
    if (s.ok()) {
#if LEMMA_L0
      cfd->mem()->SetLogNumber(logfile_number_);
      //cfd->mem()->SetLog(logs_.back().del, alive_log_files_.back().del);
      cfd->mem()->SetLog(alive_log_files_.back().del);
#endif
#if DEBUG_PRINT
      spdk_fs_->g_memtable_num += 1;
      spdk_fs_->g_memtable_size += cfd->mem()->ApproximateMemoryUsage();
#endif
