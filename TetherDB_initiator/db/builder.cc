...
Status BuildTable(
  ...
  if (iter->Valid() || !range_del_agg->IsEmpty()) {
    ...
    for (; c_iter.Valid(); c_iter.Next()) {
      ...
      //uint64_t tmp_off = c_iter.log_offset();
      //if (tmp_off < 1000) fprintf(stderr, "out offset: %lu \n", tmp_off); //lemmalemmalemma
    ...
    if (s.ok() && !empty) {
      ...
      db_options.greeter->Caching(meta, start_lpn, end_lpn);//lemma
#else
      db_options.greeter->Caching(meta);//lemma
#endif
#endif
