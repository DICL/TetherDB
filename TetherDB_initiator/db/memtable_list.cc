...
#if LEMMA_L0
void MemTableList::L0CompactionDeleteMems(std::vector<MemTable*>& to_delete) {
  InstallNewVersion();
  autovector<MemTable*> memtables_to_free;
  for(auto m : to_delete){
    current()->Remove(m, &memtables_to_free);
    UpdateCachedValuesFromMemTableListVersion();
    ResetTrimHistoryNeeded();
  }
  if(memtables_to_free.size() > 0)
    fprintf(stderr, "wow!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! memtables_to_free.size() > 0\n");
}

Status MemTableList::TryInstallL0CompactionResults(
    ColumnFamilyData* cfd, const MutableCFOptions& mutable_cf_options,
    VersionEdit* const edit, InstrumentedMutex* db_mutex,
    FSDirectory* db_directory, Compaction* compaction,
    VersionSet* vset) {
  std::vector<MemTable*> to_delete;
  for(auto l0 : *compaction->inputs(0)) {
    if (l0->log_sst != nullptr) {
      //remove log
      l0->log_sst->mem()->SetLogTrue();
#if DEBUG_PRINT
      fprintf(stderr, "L0 compaction result %lu\n", l0->log_sst->LogNumber());
#endif
      //remove mem
      to_delete.push_back(l0->log_sst->mem());
    }
  }
  const auto manifest_write_cb = [this, &to_delete](const Status& /*status*/) {
    L0CompactionDeleteMems(to_delete);
  };
  autovector<VersionEdit*> edit_list;
  edit_list.emplace_back(edit);
  return vset->LogAndApply(cfd, mutable_cf_options, edit_list, db_mutex,
                                db_directory, false, nullptr,
                                manifest_write_cb);
}
#endif
