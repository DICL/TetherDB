...
class MemTableListVersion {
 public:
  ...
  std::list<MemTable*>& GetList() {return memlist_;}
#endif
...
class MemTableList {
 public:
...
#if LEMMA_L0
  void L0CompactionDeleteMems(std::vector<MemTable*>& to_delete);

  Status TryInstallL0CompactionResults(
    ColumnFamilyData* cfd, const MutableCFOptions& mutable_cf_options,
    VersionEdit* const edit, InstrumentedMutex* db_mutex,
    FSDirectory* db_directory, Compaction* compaction,
    VersionSet* vset);

  SequenceNumber GetCurrentEarliestSequenceNumber() const {
    if (!current_->memlist_.empty()) {
      auto l_front = current_->memlist_.begin();
      std::advance(l_front, num_flush_not_started_);
      return (*l_front)->GetEarliestSequenceNumber();
    } else {
  ...
  int GetRefs() {return current_->refs_;}
#endif
