...
#if LEMMA_L0
IOStatus Writer::AddRecord(const Slice& slice, size_t &start_index, size_t &start_offset, size_t &end_index, size_t &end_offset, int &header_size_t) {
  const char* ptr = slice.data();
  size_t left = slice.size();
  ...
  do {
    ...
    if(begin) {
      start_index = block_index_;
      start_offset = block_offset_;
    }
  ...
  end_index = block_index_;
  end_offset = block_offset_;
