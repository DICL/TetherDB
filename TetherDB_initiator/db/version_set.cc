 ...
 public:
  FilePicker(std::vector<FileMetaData*>* files, const Slice& user_key,
    ...
    if (!search_ended_) {
      // Prefetch Level 0 table data to avoid cache miss if possible.
      for (unsigned int i = 0; i < (*level_files_brief_)[0].num_files; ++i) {
        if (!((*level_files_brief_)[0].files[i].file_metadata->is_wal)) {
          auto* r = (*level_files_brief_)[0].files[i].fd.table_reader;
          if (r) {
            r->Prepare(ikey);
          }
...
#if LEMMA_L0
    fprintf(stderr, "\n\nLEMMA L0 does not supprot MultiGet\n\n\n");
#endif
    // Setup member variables to search first level.
    search_ended_ = !PrepareNextLevel();
#if LEMMA_L0 == 0
      ...
      if (!file.file_metadata->is_wal) {
        merge_iter_builder->AddIterator(cfd_->table_cache()->NewIterator(
            ...
            *file.file_metadata, range_del_agg,
            ...
            /*largest_compaction_key=*/nullptr, allow_unprepared_value));
        if (should_sample) {
          sample_file_read_inc(file.file_metadata);
        }
      }
    }
#endif
...
void Version::GetTestLemma(const ReadOptions& /*read_options*/, const LookupKey& k,
...
void Version::Get(const ReadOptions& read_options, const LookupKey& k,
  ...
  while (f != nullptr) {
    ...
    *status = table_cache_->Get(
    ...
    if (!status->ok()) {
      fprintf(stderr, "WTF WTF WTF WTF 22!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
      return;
    }
    ...
    switch (get_context.State()) {
          ...
          if (do_merge && value) {
            *status = GetBlob(read_options, user_key, *value, value);
            if (!status->ok()) {
              ...
              fprintf(stderr, "WTF WTF WTF WTF 33!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \n");
              return;
            }
          }
        }

        //fprintf(stderr, "WTF WTF WTF WTF 44!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! %lu\n", fp_list[0]);
        return;
      case GetContext::kDeleted:
        // Use empty error message for speed
        *status = Status::NotFound();
        fprintf(stderr, "WTF WTF WTF WTF 55!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \n");
        return;
      case GetContext::kCorrupt:
        *status = Status::Corruption("corrupted key for ", user_key);
        fprintf(stderr, "WTF WTF WTF WTF 66!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \n");
        return;
      case GetContext::kUnexpectedBlobIndex:
        ...
        *status = Status::NotSupported(
        ...
        fprintf(stderr, "WTF WTF WTF WTF 77!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \n");
        return;
    }
    f = fp.GetNextFile();
  }
  ...
  if (GetContext::kMerge == get_context.State()) {
    ...
    if (!merge_operator_) {
      *status =  Status::InvalidArgument(
      ...
      fprintf(stderr, "WTF WTF WTF WTF 88!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \n");
      return;
    }
...
//#if LEMMA_L0
//      num_sorted_runs = num_sorted_runs/2;
//#endif
...
#if DEBUG_PRINT
  fprintf(stderr, "AddFile called!!!!!!!!!!!!!!$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ %d\n", level);//lemma
#endif
...
#if REMOTE_RANGE
  if (smallest_.size()) {
    if (internal_comparator_->Compare(smallest_, f->smallest) > 0)
      smallest_.DecodeFrom(f->smallest.Encode());
  } else {
    smallest_.DecodeFrom(f->smallest.Encode());
  }
  if (largest_.size()) {
    if (internal_comparator_->Compare(largest_, f->largest) < 0)
      largest_.DecodeFrom(f->largest.Encode());
  } else {
    largest_.DecodeFrom(f->largest.Encode());
  }
#endif
      ...
      for (int i = 0; i < static_cast<int>(versions.size()); ++i) {
        ...
        if (!s.ok()) {
          if (db_options_->paranoid_checks) {
            fprintf(stderr, "builder_guards[i]->version_builder()->LoadTableHandlers failed!! 11\n");
            break;
          }
          s = Status::OK();
        }
...
#if LEMMA_L0
    if (f.file_metadata->log_sst != nullptr) {
      result = f.file_metadata->log_sst->ApproximateOffsetOf(f.fd.GetFileSize(), key);
    } else {
#endif
...
#if LEMMA_L0
  if (f.file_metadata->log_sst != nullptr) {
    return ApproximateOffsetOf(v, f, end, caller) - ApproximateOffsetOf(v, f, start, caller);
  } else {
#endif
...
#if LEMMA_L0
          if(flevel->files[i].file_metadata->log_sst != nullptr) {
            //list[num++] = new LogSstIterator(flevel->files[i].file_metadata->log_sst, file_options_compactions);
            list[num++] = flevel->files[i].file_metadata->log_sst->mem()->NewIterator();
          } else {
