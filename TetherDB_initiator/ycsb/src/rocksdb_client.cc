...
void RocksDBClient::Load(){
  ...
  printf("------------------------------------------------------------------\n");
  printf("Load %c - IOPS: %.3lf K\n", workload_proxy_->name_str().back(), load_num_/time*1000*1000/1000);
  printf("==================================================================\n");
...
void RocksDBClient::Work(){
  ...
  printf("------------------------------------------------------------------\n");
  printf("%s %s - IOPS: %.3lf K\n", workload_proxy_->name_str().c_str(), workload_proxy_->distribution_str().c_str(), request_num_/time*1000*1000/1000);
  printf("==================================================================\n");
  fflush(stdout);
...
void RocksDBClient::RocksDBWorker(uint64_t num, int coreid, bool is_master){
  ...
  for(uint64_t i=0; i<num; i++){
    ...
    if(opt == READ){
      ...
      rocksdb::Status ec = db_->Get(read_options_, req->Key(), &r_value);
      if(!ec.ok()) {
        fprintf(stderr, "no!!!!!!!!!!!!!!!!!!!!! %s:%d: %s\n", __FILE__, __LINE__, ec.ToString().c_str());
        return;
      }
//fprintf(stderr, "search key: %s\n", req->Key().c_str());
      ...
      rocksdb::Status ec = db_->Put(write_options_, req->Key(), w_value);
      if(!ec.ok()) {
        fprintf(stderr, "no!!!!!!!!!!!!!!!!!!!!! %s:%d: %s\n", __FILE__, __LINE__, ec.ToString().c_str());
        return;
      }
      ...
      rocksdb::Slice w_value(w_value_buf, req->Length());
      //ERR(db_->Put(write_options_, req->Key(), w_value));
      ec = db_->Put(write_options_, req->Key(), w_value);
      if(!ec.ok()) {
        fprintf(stderr, "no!!!!!!!!!!!!!!!!!!!!! %s:%d: %s\n", __FILE__, __LINE__, ec.ToString().c_str());
        return;
      }
    ...
    if(opt == READ || opt == SCAN){
      ...
      total_finished_requests[coreid]++;
      read_finished[coreid]++;
...
void RocksDBClient::RocksdDBLoader(uint64_t num, int coreid){
  ...
  for(uint64_t i=0; i<num; i++){    
    ...
    rocksdb::Status ec = db_->Put(write_options_, key, w_value);
    if(!ec.ok()) {
      fprintf(stderr, "no!!!!!!!!!!!!!!!!!!!!! %s:%d: %s\n", __FILE__, __LINE__, ec.ToString().c_str());
      return;
    }
...
void RocksDBClient::Reset(){
  ...
  for (int i = 0; i < num_threads_; i++) {
    total_finished_requests[i] = 0;
    write_finished[i] = 0;
    read_finished[i] = 0;
    /*if (td_[i]) {
      memset(td_[i], 0, sizeof(thread_data));
      printf("reset called !! %lu, %lu, %lu\n", td_[i]->total_finished_requests, td_[i]->write_finished, td_[i]->read_finished);
    }*/
  }
  stop_.store(false);
}
...
void RocksDBClient::perfmon_loop() {
  ...
  logdir.append(user_name).append("/logs/rocksdb/");
  ...
  while (!stop_.load()) {
    ...
    for (int i = 0; i < num_threads_; i++) {
      total_cnt_now += total_finished_requests[i];
      write_cnt_now += write_finished[i];
      read_cnt_now += read_finished[i];
      /*if (td_[i]) {
