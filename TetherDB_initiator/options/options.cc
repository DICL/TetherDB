...
#if LEMMA_PATH
  cf_paths.push_back(DbPath({"/lemma/rocksdb",100llu<<30}));//lemma_path
  cf_paths.push_back(DbPath({"/lemma/rocksdb2",100llu<<30}));//lemma_path
  fprintf(stderr, "!!!!!!!!!!! 111 %lu\n", cf_paths.size());
#endif

#if LEMMA_L0
  fprintf(stderr, "!!!! ColumnFamilyOptions 11\n");
  std::vector<ColumnFamilyDescriptor> column_families1;
  const int kkMemtableSize = 1 << 20;
  const int kkMemtablePrefixFilterSize = 1 << 14;
  const int kkPrefixLen = 4;
  memtable_prefix_bloom_size_ratio =
    static_cast<double>(kkMemtablePrefixFilterSize) / kkMemtableSize;
  prefix_extractor.reset(
    ROCKSDB_NAMESPACE::NewFixedPrefixTransform(kkPrefixLen));
...
#if LEMMA_L0
  fprintf(stderr, "!!!! ColumnFamilyOptions 22\n");
  std::vector<ColumnFamilyDescriptor> column_families1;
  const int kkMemtableSize = 1 << 20;
  const int kkMemtablePrefixFilterSize = 1 << 14;
  const int kkPrefixLen = 4;
...
DBOptions* DBOptions::IncreaseParallelism(int total_threads) {
  fprintf(stderr, "************************************************************************************* IncreaseParallelism\n");
