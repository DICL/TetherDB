...
#include "lemma/remote_compaction_iterator.h"
...
struct RemoteCompactionJob::SubcompactionState {
  const RemoteCompaction* compaction;
  const Compaction* og_compaction;
  std::unique_ptr<RemoteCompactionIterator> c_iter;
  ...
  SubcompactionState(RemoteCompaction* c, Compaction* og_c, Slice* _start, Slice* _end, uint64_t size)
      : compaction(c), og_compaction(og_c), start(_start), end(_end), approx_size(size) {
...
struct RemoteCompactionJob::CompactionState {
  RemoteCompaction* const compaction;
  Compaction* const og_compaction;
  ...
  explicit CompactionState(RemoteCompaction* c, Compaction* og_c) : compaction(c), og_compaction(og_c) {}
...
RemoteCompactionJob::RemoteCompactionJob(
    int job_id, RemoteCompaction* compaction, Compaction* og_compaction, const ImmutableDBOptions& db_options,
    ...
    : job_id_(job_id),
      compact_(new CompactionState(compaction, og_compaction)),
...
void RemoteCompactionJob::ReportStartedCompaction(RemoteCompaction* compaction) {
  const auto* cfd = compact_->compaction->column_family_data();
  ThreadStatusUtil::SetColumnFamily(cfd, cfd->ioptions()->env,
                                    db_options_.enable_thread_tracking);
...
void RemoteCompactionJob::Prepare() {
  ...
  if (c->ShouldFormSubcompactions()) {
    ...
    for (size_t i = 0; i <= boundaries_.size(); i++) {
      ...
      compact_->sub_compact_states.emplace_back(c, compact_->og_compaction, start, end, sizes_[i]);
    }
    ...
    compact_->sub_compact_states.emplace_back(c, compact_->og_compaction, start, end, size);
  }
}
...
Status RemoteCompactionJob::Run() {
#if DEBUG_PRINT 
  fprintf(stderr, "\n!!!!! CompactionJob::Run called\n"); //lemma
#endif
  AutoThreadOperationStageUpdater stage_updater(
  ...
  for (size_t i = 1; i < compact_->sub_compact_states.size(); i++) {
    fprintf(stderr, "\n!!!!! CompactionJob::Run 1111111111 called\n"); //lemma
  ...
  fprintf(stderr, "\n!!!!! CompactionJob::Run 222222222 called\n"); //lemma
  ...
  if (status.ok()) {
    ...
    auto verify_table = [&](Status& output_status) {
      while (true) {
        ...
        //fprintf(stderr, "\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ Run() fd.GetNumber() = %lu  @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n", files_output[file_idx]->meta.fd.GetNumber());//lemmalemmalemma
...
void RemoteCompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {//lemmalemma
  ...
  std::unique_ptr<InternalIterator> input(
      versions_->MakeInputIterator(read_options, sub_compact->og_compaction,
  ...
  sub_compact->c_iter.reset(new RemoteCompactionIterator(
...
Status RemoteCompactionJob::InstallCompactionResults(
  ...
  if (!versions_->VerifyCompactionFileConsistency(compact_->og_compaction)) {
    RemoteCompaction::InputLevelSummaryBuffer inputs_summary;
  ...
  fprintf(stderr, "\n@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ input_count = %lu, output_count = %d, input_size = %lu, approxi_num = %lu @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n", input_count, output_count, input_size, input_size / compaction->max_output_file_size());
...
void RemoteCompactionJob::UpdateCompactionStats() {
  ...
  RemoteCompaction* compaction = compact_->compaction;
...
void RemoteCompactionJob::UpdateCompactionInputStatsHelper(int* num_files,
                                                     uint64_t* bytes_read,
                                                     int input_level) {
  const RemoteCompaction* compaction = compact_->compaction;
