...
RemoteCompactionIterator::RemoteCompactionIterator(
    ...
    const RemoteCompaction* compaction, const CompactionFilter* compaction_filter,
    const std::atomic<bool>* shutting_down,
    const SequenceNumber preserve_deletes_seqnum,
    const std::atomic<int>* manual_compaction_paused,
    const std::shared_ptr<Logger> info_log,
    const std::string* full_history_ts_low)
    : RemoteCompactionIterator(
...
void RemoteCompactionIterator::ResetRecordCounts() {
...
void RemoteCompactionIterator::SeekToFirst() {
  NextFromInput();
  PrepareOutput();
}

void RemoteCompactionIterator::Next() {
...
bool RemoteCompactionIterator::InvokeFilterIfNeeded(bool* need_skip,
                                              Slice* skip_until) {
...
void RemoteCompactionIterator::NextFromInput() {
  at_next_ = false;
  valid_ = false;
...
bool RemoteCompactionIterator::ExtractLargeValueIfNeededImpl() {
...
void RemoteCompactionIterator::ExtractLargeValueIfNeeded() {
...
void RemoteCompactionIterator::GarbageCollectBlobIfNeeded() {
...
void RemoteCompactionIterator::PrepareOutput() {
  if (valid_) {
    if (ikey_.type == kTypeValue) {
...
inline SequenceNumber RemoteCompactionIterator::findEarliestVisibleSnapshot(
...
inline bool RemoteCompactionIterator::ikeyNotNeededForIncrementalSnapshot() {
  return (!compaction_->preserve_deletes()) ||
         (ikey_.sequence < preserve_deletes_seqnum_);
}

bool RemoteCompactionIterator::IsInEarliestSnapshot(SequenceNumber sequence) {
  assert(snapshot_checker_ != nullptr);
...
uint64_t RemoteCompactionIterator::ComputeBlobGarbageCollectionCutoffFileNumber(
