...
#if LEMMA_L0
  char* MutableKey() { return reinterpret_cast<char*>(&next_[1]); }
#endif
...
inline const char* InlineSkipList<Comparator>::Iterator::key() const {
  assert(Valid());
  //fprintf(stderr, "InlineSkipList\n");//lemmalemma
  return node_->Key();
}

#if LEMMA_L0
template <class Comparator>
inline char* InlineSkipList<Comparator>::Iterator::mutable_key() {
  assert(Valid());
  return node_->MutableKey();
}
#endif
