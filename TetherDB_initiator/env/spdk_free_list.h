  ...
  class SPDKFreeList{
    ...
    public:
      ...
      void PrintStatus(){
        ...
        fprintf(stderr, "free list entry num: %ld, total size: %.3lf GB\n", size, (space_size * 4096 * 1.0)/(1ull<<30));
        //printf("spandb_controller ld size: %.2lf\n", spandb_controller_.GetLDLeftSpace()); //lemma
      }

      uint64_t Size(){return size_.load();}

      void Put(uint64_t start, uint64_t end){
        //spandb_controller_.IncreaseLDLeftSpace(end - start + 1); //lemma
      ...
      void Remove(uint64_t start, uint64_t end){
        ...
        //spandb_controller_.DecreaseLDLeftSpace(end - start + 1); //lemma
        std::unique_lock<std::mutex> lk(mutex_);
        for(auto it = free_list_.begin(); it != free_list_.end(); it++){
      ...
      uint64_t Get(uint64_t size){
        ...
        //spandb_controller_.DecreaseLDLeftSpace(size); //lemma
        {
          std::unique_lock<std::mutex> lk(mutex_);
          ...
          //split small entry
          //fprintf(stderr, "split small entry !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
          for(auto it = free_list_.begin(); it != free_list_.end(); it++){
          ...
          //split big entry
          //fprintf(stderr, "split big entry !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
          for(auto it = free_list_.begin(); it != free_list_.end(); it++){
            if(it->Size() > size){
              uint64_t start = it->GetStart();
              it->SetStart(start + size);
