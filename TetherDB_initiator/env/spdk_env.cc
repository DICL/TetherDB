...
void SpdkEnv::SpdkMigration(std::string dbpath, std::string data_path){
  SpdkMigrationImpl(dbpath, data_path);
}
...
#if DEBUG_PRINT
    fprintf(stderr, "!!! meta_mutex_ NewRandomAccessFile\n");//lemma_print
#endif
...
/*  SpdkFile* file = new SpdkFile(env_, fn, metadata, metadata->size_);
  result->reset(new SpdkSequentialFile(file));
  {
    MutexLock lock(&fs_mutex_);
#if DEBUG_PRINT
    fprintf(stderr, "!!! fs_mutex_ NewRandomAccessFile\n");//lemma_print
#endif
    spdk_file_system_[fn] = file;
  }
  return IOStatus::OK();*/
...
void SpdkEnv::SetLevel(const std::string& fname, int /*level*/) {
  auto fn = NormalizePath(fname);
  SPDKFileMeta* metadata = nullptr;
  {
    // 2. search in metadata
...
IOStatus SpdkEnv::NewRandomAccessFile(const std::string& fname,
                                    ...
                                    std::unique_ptr<FSRandomAccessFile>* result,
                                    IODebugContext* /**/) {
  //fprintf(stderr, "NewRandomAccessFile %s\n", fname.c_str());//lemma_print
  auto fn = NormalizePath(fname);
  {
    // 1. search in FileSystem
    MutexLock lock(&fs_mutex_);
  ...
  assert(metadata != nullptr);
  {
    ...
    MutexLock lock(&fs_mutex_);
    if (spdk_file_system_.find(fn) != spdk_file_system_.end()) {
      if (spdk_file_system_[fn]->get_ENTRY_SIZE() == LARGE_FILE_BUFFER_ENTRY_SIZE) {
        fprintf(stderr, "NewRandomAccessFile %s from existing files but something wrong!!!!\n", fname.c_str());
      }
      result->reset(new SpdkRandomAccessFile(spdk_file_system_[fn]));
      return IOStatus::OK();
    } else {
...
IOStatus SpdkEnv::ReuseWritableFile(const std::string& fname,
  ...
  //fprintf(stderr, "ReuseWritableFile %s %s\n", fname.c_str(), old_fname.c_str());//lemma_print
  if (!s.ok()) {
...
IOStatus SpdkEnv::NewWritableFile(const std::string& fname,
...
#if POS_IO
  pre_allocate_size =
      (pre_allocate_size / LARGE_FILE_BUFFER_ENTRY_SIZE + 1) * LARGE_FILE_BUFFER_ENTRY_SIZE;
#else
  pre_allocate_size =
...
#endif
    if (start_lpn == 0) {
      fprintf(stderr, "1111 %s error file %s %lu\n", __FUNCTION__, fname.c_str(), page_num);
  ...
  SpdkFile* file;
  //if (fn[fn.size()-1] == 't')
    //file = new SpdkFile(env_, fn, meta, pre_allocate_size, FILE_BUFFER_ENTRY_SIZE, is_flush, true);
  //else
    file = new SpdkFile(env_, fn, meta, pre_allocate_size, LARGE_FILE_BUFFER_ENTRY_SIZE, is_flush, true);
#else
  SpdkFile* file = new SpdkFile(env_, fn, meta, pre_allocate_size, FILE_BUFFER_ENTRY_SIZE, is_flush, true);
#endif
  // check overlap
  IOStatus s = check_overlap(start_lpn, end_lpn, fn);
  {
    MutexLock lock(&meta_mutex_);
#if DEBUG_PRINT
    fprintf(stderr, "!!! meta_mutex_ NewWritableFile\n");//lemma_print
#endif
    if (!s.ok()) {
    //lemma_check overlap   fprintf(stderr, "2222 %s error file %s\n", __FUNCTION__, fname.c_str());
      return s;
    }
    file_meta_[fn] = meta;
  }
  {
    MutexLock lock(&fs_mutex_);
#if DEBUG_PRINT
   fprintf(stderr, "!!! fs_mutex_ NewWritableFile\n");//lemma_print
#endif
    spdk_file_system_[fn] = file;
  }
...
IOStatus SpdkEnv::FileExists(const std::string& fname,
  ...
  MutexLock lock(&meta_mutex_);
#if DEBUG_PRINT
  fprintf(stderr, "!!! meta_mutex_ FileExists\n");//lemma_print
#endif
...
IOStatus SpdkEnv::GetChildren(const std::string& dir,
  ...
  for(auto m : file_meta_) {
    result->push_back(m.first.substr(15));
  }
  if(dir == "") printf("\n");
  return IOStatus::OK();
}
...
IOStatus SpdkEnv::DeleteFileInternal(const std::string& fname) {
  assert(fname == NormalizePath(fname));
  {
    ...
    if (fs_pair != spdk_file_system_.end()) {
      ...
      fprintf(stderr, "!!! DeleteFileInternal %s %lu\n", fname.c_str(), fs_pair->second->GetRef());//lemma_print
  ...
  uint64_t end_lpn = 0;
  {
    ...
    MutexLock lock(&meta_mutex_);
#if DEBUG_PRINT
    fprintf(stderr, "!!! meta_mutex_ DeleteFileInternal\n");//lemma_print
#endif
...
IOStatus SpdkEnv::DeleteDir(const std::string& dirname,
  ...
  for (auto m : file_meta_) {
    files.push_back(m.second->fname_);
  }
  for (auto fn : files) {
...
IOStatus SpdkEnv::GetFileSize(const std::string& fname,
  ...
  MutexLock lock(&meta_mutex_);
#if DEBUG_PRINT
  fprintf(stderr, "!!! meta_mutex_ GetFileSize\n");//lemma_print
#endif
  auto iter = file_meta_.find(fn);
  if (iter == file_meta_.end()) {
    fprintf(stderr, "GetFileSize err 11 %s\n", fname.c_str());
    return IOStatus::IOError(fn, "SPDK-env: file not found");
  }
...
IOStatus SpdkEnv::GetFileAddr(const std::string& fname,
  ...
  MutexLock lock(&meta_mutex_);
#if DEBUG_PRINT
  fprintf(stderr, "!!! meta_mutex_ GetFileAddr\n");//lemma_print
#endif
  auto iter = file_meta_.find(fn);
  if (iter == file_meta_.end()) {
    fprintf(stderr, "GetFileSize err 22 %s\n", fname.c_str());
    return IOStatus::IOError(fn, "SPDK-env: file not found");
  }
...
uint64_t SpdkEnv::GetFreeBlocks(uint64_t pre_allocate_size,
  ...
  for(int i=0; i<file_num; i++){
    ...
    if (start_lpns[i] == 0) {
      fprintf(stderr, "1111 %s error %lu\n", __FUNCTION__, page_num);
    }
  }
...
IOStatus SpdkEnv::NewWritableFileFromFree(const std::string& fname,
                                ...
                                uint64_t pre_allocate_size,
                                uint64_t start_lpn) {
  //fprintf(stderr, "NewWritableFileFromFree %s\n", fname.c_str());//lemma_print
  auto fn = NormalizePath(fname);
  bool is_flush = false;
  if (pre_allocate_size == 0) {
    pre_allocate_size = 1<<30;//env_options.allocate_size; lemmalemmalemma
...
#endif
  // check overlap
  //lemma_check overlap IOStatus s = check_overlap(start_lpn, end_lpn, fn);
  {
    MutexLock lock(&meta_mutex_);
#if DEBUG_PRINT
    fprintf(stderr, "!!! meta_mutex_ NewWritableFileFromFree\n");//lemma_print
#endif
    //lemma_check overlap if (!s.ok()) {
    //lemma_check overlap   fprintf(stderr, "2222 %s error file %s\n", __FUNCTION__, fname.c_str());
    //lemma_check overlap   return s;
    //lemma_check overlap }
    file_meta_[fn] = meta;
  }
  {
    MutexLock lock(&fs_mutex_);
#if DEBUG_PRINT
    fprintf(stderr, "!!! fs_mutex_ NewWritableFileFromFree\n");//lemma_print
#endif
    spdk_file_system_[fn] = file;
  }
  //fprintf(stderr, "NewWritableFile %s from device\n", fname.c_str());
...
IOStatus SpdkEnv::GetFileModificationTime(const std::string& fname,
  ...
  MutexLock lock(&meta_mutex_);
#if DEBUG_PRINT
  fprintf(stderr, "!!! meta_mutex_ GetFileModificationTime\n");//lemma_print
#endif
  auto iter = file_meta_.find(fn);
  if (iter == file_meta_.end()) {
    fprintf(stderr, "%s error file %s\n", __FUNCTION__, fname.c_str());
...
IOStatus SpdkEnv::RenameFile(const std::string& src,
                           ...
                           const IOOptions& /**/,
                           IODebugContext* /**/) {
  //fprintf(stderr, "RenameFile %s %s\n", src.c_str(), dest.c_str());//lemma_print
  ...
  auto t = NormalizePath(dest);
  {
    ...
    MutexLock lock(&meta_mutex_);
#if DEBUG_PRINT
    fprintf(stderr, "!!! meta_mutex_ RenameFile\n");//lemma_print
#endif
  ...
  }
  {
    ...
    MutexLock lock(&fs_mutex_);
#if DEBUG_PRINT
    fprintf(stderr, "!!! fs_mutex_ RenameFile\n");//lemma_print
#endif
