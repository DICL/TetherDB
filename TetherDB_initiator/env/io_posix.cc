...
IOStatus PosixSequentialFile::InvalidateCache(size_t offset, size_t length) {
  fprintf(stderr, "PosixRandomAccessFile::InvalidateCache\n");
#ifndef OS_LINUX
  (void)offset;
  (void)length;
  return IOStatus::OK();
...
size_t PosixHelper::GetLogicalBlockSizeOfFd(int fd) {
  ...
  FILE* fp;
  size_t size = 0;
  fprintf(stderr, "io_posix open 22 !! %s\n", fname.c_str());
  fp = fopen(fname.c_str(), "r");
  fprintf(stderr, "io_posix open 33 !! %s\n", fname.c_str());
  if (fp != nullptr) {
    char* line = nullptr;
    size_t len = 0;
...
#if defined(ROCKSDB_FALLOCATE_PRESENTT) && defined(FALLOC_FL_PUNCH_HOLE) && \
    !defined(TRAVIS)
...
IOStatus PosixWritableFile::Sync(const IOOptions& /*opts*/,
                                 IODebugContext* /*dbg*/) {
  //fprintf(stderr, "rocks fdatasync func called!!! %s\n", filename_.c_str());//lemma
  if (fdatasync(fd_) < 0) {
...
IOStatus PosixWritableFile::Fsync(const IOOptions& /*opts*/,
                                  IODebugContext* /*dbg*/) {
  //fprintf(stderr, "rocks fsync func called!!! %s\n", filename_.c_str());//lemma
  if (fsync(fd_) < 0) {
    return IOError("While fsync", filename_, errno);
  }
  return IOStatus::OK();
...
PosixDirectory::~PosixDirectory() { 
  //fprintf(stderr, "!!!!!!!!dir closed !!!!!!!!!!!\n");//lemma
  close(fd_);
}

IOStatus PosixDirectory::Fsync(const IOOptions& /*opts*/,
                               IODebugContext* /*dbg*/) {
  //fprintf(stderr, "!!!!!!!!dir fsync 11111 !!!!!!!!!!!\n");//lemma
#ifndef OS_AIX
  //fprintf(stderr, "!!!!!!!!dir fsync 22222 !!!!!!!!!!!\n");//lemma
  if (fsync(fd_) == -1) {
    return IOError("While fsync", "a directory", errno);
  }
  //fprintf(stderr, "!!!!!!!!dir fsync 33333 !!!!!!!!!!!\n");//lemma
#endif
  return IOStatus::OK();
}
}  // namespace ROCKSDB_NAMESPACE
