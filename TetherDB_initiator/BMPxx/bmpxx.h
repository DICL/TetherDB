...
namespace bmpxx
{
  struct BmpDesc
  {
    ...
    BmpDesc(uint32_t _width, uint32_t _height, uint8_t _channels)
  ...
  public:
    ...
    static std::pair<std::vector<uint8_t>, BmpDesc> crop(std::vector<uint8_t> inputImage, BmpDesc desc, int x1, int y1, int x2, int y2);
    static std::vector<uint8_t> rotate(std::vector<uint8_t> inputImage, BmpDesc desc, double degree);
  };
