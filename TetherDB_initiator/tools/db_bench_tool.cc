    ...
    if (done_ >= next_report_) {
      if (!FLAGS_stats_interval) {
        ...
        if (FLAGS_stats_interval_seconds &&
          ...
          if (FLAGS_statistics) {
            std::string db_stat = dbstats->ToString();

            std::string miss_prefix = "rocksdb.block.cache.miss COUNT : ";
            std::string hit_prefix = "rocksdb.block.cache.hit COUNT : ";
            std::string add_prefix = "rocksdb.block.cache.add COUNT : ";

            size_t miss_pos = db_stat.find(miss_prefix);
            size_t hit_pos = db_stat.find(hit_prefix);
            size_t add_pos = db_stat.find(add_prefix);

            std::string miss_str = db_stat.substr(miss_pos, hit_pos - miss_pos - 1);
            std::string hit_str = db_stat.substr(hit_pos, add_pos - hit_pos - 1);

            unsigned long miss_cnt = strtoul(miss_str.substr(miss_str.find(':') + 2).c_str(), nullptr, 0);
            unsigned long hit_cnt = strtoul(hit_str.substr(hit_str.find(':') + 2).c_str(), nullptr, 0);
            double hit_ratio = hit_cnt ? (1.0 * hit_cnt) / (hit_cnt + miss_cnt) : 0;
            unsigned long cache_usage, cache_capacity;

            db->GetIntProperty(DB::Properties::kBlockCacheUsage, &cache_usage);
            db->GetIntProperty(DB::Properties::kBlockCacheCapacity, &cache_capacity);


            if (id_ == 0) {
              fprintf(stderr,
                      "%s ... thread %d: (%" PRIu64 ",%" PRIu64 ") ops and "
                      "(%.1f,%.1f) ops/second in (%.6f,%.6f) seconds, hitratio=%.2f (hit=%lu, miss=%lu), usage=%lu, capacity=%lu\n",
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
  ...
  void Run() {
    ...
    open_options_.greeter = new GreeterClient("10.0.0.90:50052");
#endif
#if ROCKSDB_SPDK
    open_options_.spdk_open_mode = 0;
    open_options_.SetSpdkEnv("trtype:RDMA adrfam:IPv4 traddr:10.0.0.90 trsvcid:4421 subnqn:nqn.2019-04.pos:subsystem2");
#endif
    Open(&open_options_);
    PrintHeader();
 ...
 private:
  ...
  Stats RunBenchmark(int n, Slice name,
    ...
    if (FLAGS_report_interval_seconds > 0) {
      std::string file_name = name.ToString() + std::__cxx11::to_string(work_count) + FLAGS_report_file;
      reporter_agent.reset(new ReporterAgent(FLAGS_env, file_name,
